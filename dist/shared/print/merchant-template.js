...
exports.MERCHANT_TEMPLATE_LABEL_FIELDS = exports.MERCHANT_TEMPLATE_BLOCK_KINDS = exports.MERCHANT_TEMPLATE_EXPORT_SCHEMA_VERSION = exports.MERCHANT_TEMPLATE_EXPORT_FORMAT = exports.MAX_MERCHANT_TEMPLATE_ENVELOPE_BYTES = exports.MAX_MERCHANT_TEMPLATE_PAYLOAD_BYTES = exports.MERCHANT_TEMPLATE_SCHEMA_VERSION = exports.MERCHANT_TEMPLATE_DOCUMENT_TYPES = exports.MERCHANT_TEMPLATE_FORMAT = void 0;
exports.validateMerchantTemplateEnvelope = validateMerchantTemplateEnvelope;
exports.validateMerchantTemplate = validateMerchantTemplate;
exports.validateMerchantTemplateText = validateMerchantTemplateText;
exports.serializeMerchantTemplatePayload = serializeMerchantTemplatePayload;
exports.applyMerchantTemplate = applyMerchantTemplate;
// ---------------------------------------------------------------------------
// Constants (public-facing contract — stable values)
// ---------------------------------------------------------------------------
/** Discriminator stored inside every merchant template payload. */
exports.MERCHANT_TEMPLATE_FORMAT = 'flocafe-merchant-print-template';
/** Supported document types (v1 ships receipts only). */
exports.MERCHANT_TEMPLATE_DOCUMENT_TYPES = ['receipt'];
/** Current schema major version. Bumping this is a breaking contract change. */
exports.MERCHANT_TEMPLATE_SCHEMA_VERSION = 1;
/** Hard payload size cap (~256 KB) enforced on write/import. */
exports.MAX_MERCHANT_TEMPLATE_PAYLOAD_BYTES = 256 * 1024;
/** Same cap applied to the whole offline transfer ENVELOPE (#448). */
exports.MAX_MERCHANT_TEMPLATE_ENVELOPE_BYTES = 256 * 1024;
/**
 ...
 */
exports.MERCHANT_TEMPLATE_EXPORT_FORMAT = 'flocafe-merchant-template';
/** Current transfer-envelope schema major version. Breaking when bumped. */
exports.MERCHANT_TEMPLATE_EXPORT_SCHEMA_VERSION = 1;
/** Ordered union of every PrintDocument v1 block kind — the allowed set. */
exports.MERCHANT_TEMPLATE_BLOCK_KINDS = [
    'business-header',
    'customer',
    'document-meta',
    'item-table',
 ...
 */
exports.MERCHANT_TEMPLATE_LABEL_FIELDS = Object.freeze({
    'business-header': [],
 ...
 */
function validateMerchantTemplateEnvelope(value) {
    const errors = [];
    ...
    if (value.format !== exports.MERCHANT_TEMPLATE_EXPORT_FORMAT) {
        reject(errors, `root.format: expected "${exports.MERCHANT_TEMPLATE_EXPORT_FORMAT}", got ${JSON.stringify(value.format)}`);
    }
    if (value.schemaVersion !== exports.MERCHANT_TEMPLATE_EXPORT_SCHEMA_VERSION) {
        reject(errors, `root.schemaVersion: unsupported transfer-file version ${JSON.stringify(value.schemaVersion)}; this build supports major version ${exports.MERCHANT_TEMPLATE_EXPORT_SCHEMA_VERSION} only`);
    }
    ...
    return {
        ...
        envelope: {
            ...
            ...(origin ? { origin } : {}),
            claimedChecksum: value.checksum,
...
function reject(accumulated, message) {
 ...
 */
function validateMerchantTemplate(value) {
    const errors = [];
    if (!isPlainObject(value)) {
    ...
    if (value.format !== exports.MERCHANT_TEMPLATE_FORMAT) {
        reject(errors, `root.format: expected "${exports.MERCHANT_TEMPLATE_FORMAT}", got ${JSON.stringify(value.format)}`);
    }
    // --- Document type --------------------------------------------------------
    if (!exports.MERCHANT_TEMPLATE_DOCUMENT_TYPES.includes(value.documentType)) {
        reject(errors, `root.documentType: unsupported document type ${JSON.stringify(value.documentType)} (supported: ${exports.MERCHANT_TEMPLATE_DOCUMENT_TYPES.join(', ')})`);
    }
    // --- Schema version gate (fail-closed on unknown majors) ------------------
    if (value.schemaVersion !== exports.MERCHANT_TEMPLATE_SCHEMA_VERSION) {
        reject(errors, `root.schemaVersion: unsupported schema version ${JSON.stringify(value.schemaVersion)}; this build supports major version ${exports.MERCHANT_TEMPLATE_SCHEMA_VERSION} only`);
    }
    // --- Blocks ---------------------------------------------------------------
    if (!Array.isArray(value.blocks)) {
        ...
        value.blocks.forEach((entry, index) => {
            ...
            if (typeof kind !== 'string' || !exports.MERCHANT_TEMPLATE_BLOCK_KINDS.includes(kind)) {
                reject(errors, `${at}.kind: unknown block kind ${JSON.stringify(kind)} (allowed: ${exports.MERCHANT_TEMPLATE_BLOCK_KINDS.join(', ')})`);
                return;
            }
            ...
            if (entry.labels !== undefined) {
                if (!isPlainObject(entry.labels)) {
                    ...
                    const allowedLabels = exports.MERCHANT_TEMPLATE_LABEL_FIELDS[kind];
 ...
 */
function validateMerchantTemplateText(raw) {
    const byteLength = new TextEncoder().encode(raw).length;
    if (byteLength > exports.MAX_MERCHANT_TEMPLATE_PAYLOAD_BYTES) {
        return {
            ok: false,
            errors: [`payload is ${byteLength} bytes; the maximum allowed size is ${exports.MAX_MERCHANT_TEMPLATE_PAYLOAD_BYTES} bytes`],
        };
    }
    let parsed;
    try {
        ...
        return { ok: false, errors: [`payload is not valid JSON: ${error.message}`] };
    }
 ...
 */
function serializeMerchantTemplatePayload(payload) {
 ...
 */
function overrideLabel(label, text) {
...
function applyLabelOverrides(block, labels) {
    switch (block.kind) {
        case 'document-meta': {
            return Object.freeze({
                ...block,
 ...
 */
function applyMerchantTemplate(document, payload) {
    const byKind = new Map(document.blocks.map((block) => [block.kind, block]));
    const blocks = [];
...
//# sourceMappingURL=merchant-template.js.map