...
exports.resolveDirectionSpec = resolveDirectionSpec;
exports.resolveValueDirection = resolveValueDirection;
exports.resolveScopeDirection = resolveScopeDirection;
exports.containsRtlScript = containsRtlScript;
exports.isLtrIsland = isLtrIsland;
/** Build the per-scope direction spec for a base document direction. */
function resolveDirectionSpec(base) {
 ...
 */
function resolveValueDirection(text, base) {
    return isLtrIsland(text) ? 'ltr' : base;
}
/** Scope-aware direction resolution. */
function resolveScopeDirection(scope, base, value) {
    if (scope === 'value') {
 ...
 */
function isLtrIsland(text) {
    if (typeof text !== 'string')
        return false;
    const value = text.trim();
    if (value.length === 0 || value.length > 64)
        return false;
    if (hasRtlScript(value))
        return false;
    if (URL_RE.test(value))
        return true;
    if (EMAIL_RE.test(value))
        return true;
    if (PHONE_RE.test(value) && hasDigit(value))
        return true;
    if (AMOUNT_RE.test(value) && hasDigit(value))
        return true;
    ...
    if (IDENTIFIER_RE.test(value) && hasDigit(value) && value.split(/\s+/).length <= 3)
        return true;
    return false;
}
