...
exports.KDS_VOIDED_ITEM_VISIBILITY_MS = exports.SchemaVersionMismatchError = exports.MIGRATIONS = exports.MAINTENANCE_DRAIN_TIMEOUT_MS = void 0;
exports.throwIfDatabaseMaintenanceAborted = throwIfDatabaseMaintenanceAborted;
exports.beginDatabaseShutdown = beginDatabaseShutdown;
exports.withDatabaseRequest = withDatabaseRequest;
exports.registerDatabaseMaintenanceStartListener = registerDatabaseMaintenanceStartListener;
exports.registerDatabaseMaintenanceEndListener = registerDatabaseMaintenanceEndListener;
exports.isDatabaseMaintenanceActive = isDatabaseMaintenanceActive;
exports.databaseMaintenanceMiddleware = databaseMaintenanceMiddleware;
exports.withDatabaseMaintenanceLock = withDatabaseMaintenanceLock;
exports.getSettingValue = getSettingValue;
exports.upsertSettings = upsertSettings;
exports.getDbHealth = getDbHealth;
exports.getDbPath = getDbPath;
exports.initDatabase = initDatabase;
exports.ensureCloudIdentity = ensureCloudIdentity;
exports.getCachedPairingCode = getCachedPairingCode;
exports.setCachedPairingCode = setCachedPairingCode;
exports.ensureTelemetryAnonId = ensureTelemetryAnonId;
exports.isTelemetryEnabled = isTelemetryEnabled;
exports.isDiagnosticsConsentEnabled = isDiagnosticsConsentEnabled;
exports.isKdsEnabled = isKdsEnabled;
exports.isServerAppEnabled = isServerAppEnabled;
exports.isKotPrintingEnabled = isKotPrintingEnabled;
exports.upsertTelemetryLastPing = upsertTelemetryLastPing;
exports.withTxn = withTxn;
exports.appendJsonArray = appendJsonArray;
exports.getDatabase = getDatabase;
exports.waitForDatabaseRequests = waitForDatabaseRequests;
exports.closeDatabase = closeDatabase;
exports.createBackupUnlocked = createBackupUnlocked;
exports.createBackup = createBackup;
exports.resetDatabaseWithBackup = resetDatabaseWithBackup;
exports.listBackups = listBackups;
exports.deleteBackup = deleteBackup;
exports.isManagedBackupFile = isManagedBackupFile;
exports.getTables = getTables;
exports.captureKitchenStationSecurityState = captureKitchenStationSecurityState;
exports.captureRestoreProtectedSettings = captureRestoreProtectedSettings;
exports.mergeRestoreProtectedSettings = mergeRestoreProtectedSettings;
exports.captureRestoreOutboxState = captureRestoreOutboxState;
exports.mergeRestoreOutboxState = mergeRestoreOutboxState;
exports.captureKdsEnabledSetting = captureKdsEnabledSetting;
exports.mergeKdsEnabledSetting = mergeKdsEnabledSetting;
exports.captureUserStationSecurityState = captureUserStationSecurityState;
exports.mergeUserStationSecurityState = mergeUserStationSecurityState;
exports.getUserKdsStationIds = getUserKdsStationIds;
exports.getKdsStationCategoryIds = getKdsStationCategoryIds;
exports.getKdsStationRoutingScope = getKdsStationRoutingScope;
exports.getKdsStationRoutingCategoryIds = getKdsStationRoutingCategoryIds;
exports.isKdsStationItemAllowed = isKdsStationItemAllowed;
exports.hasUserKdsStationAssignments = hasUserKdsStationAssignments;
exports.captureUserSecurityState = captureUserSecurityState;
exports.mergeUserSecurityState = mergeUserSecurityState;
exports.restoreBackup = restoreBackup;
exports.getForeignKeyViolationKeys = getForeignKeyViolationKeys;
exports.isSafeIdentifier = isSafeIdentifier;
exports.getSchemaVersionFromBackup = getSchemaVersionFromBackup;
exports.getCurrentSchemaVersion = getCurrentSchemaVersion;
exports.buildIdealSchemaDb = buildIdealSchemaDb;
exports.generateShortId = generateShortId;
exports.dateStampInTimezone = dateStampInTimezone;
exports.generateOrderNumber = generateOrderNumber;
exports.generateBillNumber = generateBillNumber;
exports.now = now;
exports.parseDbTimestamp = parseDbTimestamp;
exports.utcTodayDate = utcTodayDate;
exports.localDateInTimezone = localDateInTimezone;
exports.dayBoundsInTimezone = dayBoundsInTimezone;
exports.utcDayBounds = utcDayBounds;
exports.verifyPin = verifyPin;
exports.isVoidedItemKdsVisible = isVoidedItemKdsVisible;
exports.projectKdsOrder = projectKdsOrder;
exports.projectKdsItem = projectKdsItem;
exports.projectKdsStation = projectKdsStation;
exports.insertOrderItemAddons = insertOrderItemAddons;
exports.parseItemJson = parseItemJson;
exports.attachEffectiveAddons = attachEffectiveAddons;
exports.parseRowJson = parseRowJson;
...
// No Electron dependency: db.ts runs as plain Node.js. "Packaged" and
// "userData" concepts that used to come from Electron's `app` object are
// now derived from env vars / NODE_ENV, matching encryption.ts's pattern.
function isAppPackaged() {
    return process.env.NODE_ENV === 'production' || process.env.FLO_PACKAGED === 'true';
}
function getAppVersion() {
    return String(require('../../package.json').version);
}
function getAppPath(key) {
    if (key === 'userData') {
        return process.env.APP_DATA || process.env.FLO_DATA_DIR || path.join(os.homedir(), '.flo-desktop');
    }
    if (key === 'documents')
        return os.homedir();
    return os.homedir();
}
const USER_ROLE_SQL_CHECK = `CHECK (role IN (${role_permissions_1.ROLE_KEYS.map((role) => `'${role}'`).join(', ')}))`;
let db;
...
let databaseMaintenanceTail = Promise.resolve();
...
let maintenanceRequestWaiters = [];
let maintenanceDrainWaiters = [];
let databaseIdleWaiters = [];
const databaseMaintenanceStartListeners = new Set();
const databaseMaintenanceEndListeners = new Set();
function releaseMaintenanceDrainWaiters() {
...
function releaseMaintenanceRequestWaiters() {
    if (activeDatabaseRequests !== 0 || databaseMaintenanceActive || databaseMaintenancePending !== 0)
        return;
    const waiters = maintenanceRequestWaiters;
    maintenanceRequestWaiters = [];
    waiters.forEach((resolve) => resolve());
}
function releaseDatabaseIdleWaiters() {
    if (activeDatabaseRequests !== 0 || databaseMaintenanceActive || databaseMaintenancePending !== 0)
        return;
...
function createMaintenanceAbortError() {
    const error = new Error('Database maintenance cancelled during shutdown');
    error.code = 'ERR_SHUTDOWN_ABORTED';
    return error;
}
function throwIfDatabaseMaintenanceAborted(signal) {
    if (signal?.aborted)
        throw createMaintenanceAbortError();
}
function createDatabaseShutdownError() {
    const error = new Error('Database access is closed during shutdown');
    error.code = 'ERR_SHUTDOWN_ABORTED';
    return error;
}
function createDatabaseShutdownTimeoutError() {
    const error = new Error(`Database shutdown timed out after ${shutdown_1.SHUTDOWN_TIMEOUT_MS}ms`);
...
function createDatabaseMissingError(dbPath) {
    const error = new Error(`Database file is missing at ${dbPath}, but this install was previously initialized. ` +
 ...
 */
exports.MAINTENANCE_DRAIN_TIMEOUT_MS = shutdown_1.SHUTDOWN_TIMEOUT_MS;
function createMaintenanceDrainTimeoutError(timeoutMs) {
    const error = new Error(`Database maintenance timed out after ${timeoutMs}ms waiting for active requests to drain`);
    error.code = 'ERR_MAINTENANCE_DRAIN_TIMEOUT';
    return error;
}
function beginDatabaseShutdown() {
    databaseShutdownRequested = true;
    databaseShutdownController.abort();
}
function getMaintenanceSignal(signal) {
    return signal ? AbortSignal.any([signal, databaseShutdownController.signal]) : databaseShutdownController.signal;
}
function waitForActiveDatabaseRequests(signal, timeoutMs) {
    if (activeDatabaseRequests === 0)
    ...
    return new Promise((resolve, reject) => {
        ...
        const onAbort = () => {
            ...
            if (index >= 0)
                maintenanceDrainWaiters.splice(index, 1);
            cleanup();
...
function withDatabaseRequest(operation, signal) {
    if (databaseShutdownRequested || signal?.aborted)
        return Promise.reject(createDatabaseShutdownError());
    const run = () => {
    ...
    return new Promise((resolve, reject) => {
        ...
        const onAbort = () => {
            ...
            if (index >= 0)
                maintenanceRequestWaiters.splice(index, 1);
            signal?.removeEventListener('abort', onAbort);
            reject(createMaintenanceAbortError());
        };
        waiter = () => {
...
function isDatabaseMaintenanceRoute(req) {
    return DATABASE_MAINTENANCE_ROUTES.has(`${req.method} ${req.path}`);
}
function databaseMaintenanceMiddleware(req, res, next) {
    if (databaseShutdownRequested) {
...
function withDatabaseMaintenanceLock(operation, signal, timeoutMs) {
    if (databaseShutdownRequested)
        return Promise.reject(createMaintenanceAbortError());
    const maintenanceSignal = getMaintenanceSignal(signal);
    const previous = databaseMaintenanceTail;
    databaseMaintenancePending += 1;
    let started = false;
    let release;
    databaseMaintenanceTail = new Promise((resolve) => { release = resolve; });
    const queued = previous.then(async () => {
        if (maintenanceSignal.aborted || databaseShutdownRequested)
            throw createMaintenanceAbortError();
        started = true;
        ...
        try {
            ...
            if (activeDatabaseRequests > 0) {
                await waitForActiveDatabaseRequests(maintenanceSignal, timeoutMs ?? exports.MAINTENANCE_DRAIN_TIMEOUT_MS);
            }
            if (maintenanceSignal.aborted || databaseShutdownRequested)
                throw createMaintenanceAbortError();
...
function getDbHealth() {
    ...
    if (dbHealthError)
        return { ok: false, error: dbHealthError };
    return { ok: true };
}
...
function getDbPath() {
    // Native Playwright owns this path for its disposable local Electron run.
    // It is intentionally opt-in and has no effect on normal desktop installs.
    if (process.env.FLO_E2E_DB_PATH)
        return path.resolve(process.env.FLO_E2E_DB_PATH);
    const projectRoot = path.basename(path.dirname(__dirname)) === 'dist'
        ? path.resolve(__dirname, '../..')
        : path.resolve(__dirname, '..');
    // Electron uses its userData directory.
    // Plain Node uses the project root.
    const userDataPath = isAppPackaged() ? getAppPath('userData') : projectRoot;
    return path.join(userDataPath, 'flo.db');
}
...
function getBackupDir() {
    const userDataPath = isAppPackaged() ? getAppPath('userData') : path.resolve(__dirname, '..');
    return path.join(userDataPath, 'backups');
}
...
// alone.
function getDbInitializedMarkerPath() {
...
function writeReplacementJournal(journalPath, journal) {
...
function isLiveDatabaseTarget(candidatePath, dbPath) {
    const normalize = (value) => process.platform === 'win32' || process.platform === 'darwin' ? value.toLowerCase() : value;
    if (normalize(path.resolve(candidatePath)) === normalize(path.resolve(dbPath)))
        return true;
    try {
        const candidateStat = fs.statSync(candidatePath);
        const dbStat = fs.statSync(dbPath);
        if (candidateStat.dev === dbStat.dev && candidateStat.ino === dbStat.ino)
            return true;
    }
    catch { }
    try {
...
function removeReplacementArtifacts(journalPath, recoveryPath) {
...
let recoverySchemaReference = null;
let buildingIdealSchema = false;
function getRecoverySchemaReference() {
    if (recoverySchemaReference)
...
function normalizedSchemaDefinitions(dbInstance) {
...
function isHealthyDatabaseFile(filePath, allowedForeignKeyViolations = undefined, requireMetadata = true) {
    try {
        const candidate = new better_sqlite3_1.default(filePath, { readonly: true, fileMustExist: true });
        const integrity = candidate.prepare('PRAGMA integrity_check').get().integrity_check === 'ok';
        ...
        let metadata;
        try {
            metadata = candidate.prepare("SELECT value FROM _flo_meta WHERE key = 'schema_version'").get();
        }
        catch { }
        ...
        const supportedVersion = exports.MIGRATIONS[exports.MIGRATIONS.length - 1]?.version || 0;
        const idealDb = buildIdealSchemaDb();
...
function removeOlderReplacementJournals(journals, dbPath, backupDir) {
...
function recoverInterruptedDatabaseReplacement(dbPath, backupDir) {
    let journals = [];
    try {
...
function initDatabase(recoverInterruptedReplacement = true, allowDuringShutdown = false) {
    if (databaseShutdownRequested && !allowDuringShutdown)
        throw createDatabaseShutdownError();
    ...
    if (recoverInterruptedReplacement)
        recoverInterruptedDatabaseReplacement(dbPath, backupDir);
    ...
    const isProductionStartup = recoverInterruptedReplacement &&
        isAppPackaged() &&
        !process.env.FLO_E2E_DB_PATH; //extra
    ...
    //if (app.isPackaged && !process.env.FLO_E2E_DB_PATH) { //extra
    if (isAppPackaged() && !process.env.FLO_E2E_DB_PATH) {
        ...
        try {
            ...
            try {
                ...
                electron_log_1.default.error('[DB] Failed to write .flo-db-initialized marker after retry — the #278 ' +
                    'missing-database guard will not protect this install until a future boot succeeds:', retryErr);
            }
        }
    }
}
...
function setCachedPairingCode(code, expiresAt) {
...
function ensureTelemetryAnonId() {
 ...
 */
function isDiagnosticsConsentEnabled() {
 ...
 */
function isKotPrintingEnabled() {
    return getSettingValue('kot_printing_enabled') !== 'false';
}
function upsertTelemetryLastPing() {
...
function appendJsonArray(table, idColumn, idValue, column, value) {
    ...
    const row = db.prepare(`SELECT ${column} AS v FROM ${table} WHERE ${idColumn} = ?`).get(idValue);
    let arr = [];
    if (row && row.v) {
        try {
...
function repairSequences() {
    try {
        const collectSequenceMax = (table, numberColumn, pattern) => {
            const rows = db.prepare(`SELECT ${numberColumn} AS value FROM ${table} WHERE ${numberColumn} IS NOT NULL`).all();
        ...
        for (const row of orderRows) {
            if (!row.date || !row.max_val)
                continue;
            const existing = db.prepare(`SELECT current_value FROM sequences WHERE name = 'orders' AND date = ?`).get(row.date);
            if (!existing) {
        ...
        for (const row of billRows) {
            if (!row.date || !row.max_val)
                continue;
            const existing = db.prepare(`SELECT current_value FROM sequences WHERE name = 'bills' AND date = ?`).get(row.date);
            if (!existing) {
...
function autoRepairPaymentDetails() {
    try {
        const rows = db.prepare(`SELECT id, payment_details, paid_amount FROM bills WHERE payment_details IS NOT NULL AND payment_details != ''`).all();
        const toFix = [];
        for (const row of rows) {
            try {
...
function autoRepairDefaultPrinter() {
    try {
...
function waitForDatabaseRequests(timeoutMs) {
    if (activeDatabaseRequests === 0 && !databaseMaintenanceActive && databaseMaintenancePending === 0)
        return Promise.resolve();
    const drain = new Promise((resolve) => databaseIdleWaiters.push(resolve));
    const effectiveTimeoutMs = timeoutMs ?? (databaseShutdownRequested ? shutdown_1.SHUTDOWN_TIMEOUT_MS : undefined);
    if (effectiveTimeoutMs === undefined)
        return drain;
    return new Promise((resolve, reject) => {
        const timeout = setTimeout(() => reject(createDatabaseShutdownTimeoutError()), effectiveTimeoutMs);
        drain.then(() => { clearTimeout(timeout); resolve(); }, (error) => { clearTimeout(timeout); reject(error); });
    });
}
...
async function createBackupUnlocked(targetPath, signal) {
    ...
    try {
      ...
      `);
        ...
        if (!completed) {
            for (const filePath of [tempPath, stagedTargetPath, `${tempPath}-wal`, `${tempPath}-shm`].filter((value) => Boolean(value))) {
                try {
                    if (pathEntryExists(filePath))
                        fs.unlinkSync(filePath);
                }
...
function createBackup(targetPath, signal) {
 ...
 */
async function resetDatabaseWithBackup(signal) {
    return withDatabaseMaintenanceLock(async (maintenanceSignal) => {
        ...
        try {
            ...
            try {
                ...
                throw new Error(`Database reset failed: ${error?.message || 'unknown error'}; ` +
...
function parseCanonicalSchemaVersion(value) {
    if (typeof value !== 'string' || !/^(?:0|[1-9]\d*)$/.test(value))
        return null;
    const parsed = Number(value);
    return Number.isSafeInteger(parsed) ? parsed : null;
}
function readBackupSchemaVersion(fullPath) {
    let backupDb;
    try {
        backupDb = new better_sqlite3_1.default(fullPath, { readonly: true, fileMustExist: true });
        const row = backupDb.prepare(`SELECT value FROM _flo_meta WHERE key = 'schema_version'`).get();
...
function listBackups() {
    ...
    return fs.readdirSync(backupDir)
        ...
        .map((fileName) => ({ fileName, fullPath: (0, path_containment_1.resolveContainedPath)(backupDir, fileName) }))
        .filter((entry) => {
        if (!entry.fullPath)
            return false;
        try {
            return fs.lstatSync(entry.fullPath).isFile();
        }
        catch {
            return false;
        }
        ...
        return {
            ...
            kind: (fileName.includes('-pre-v') ? 'auto' : 'manual'),
...
function deleteBackup(fileName) {
    ...
    const backupDir = getBackupDir();
    const fullPath = (0, path_containment_1.resolveContainedPath)(backupDir, fileName);
 ...
 */
function isManagedBackupFile(candidatePath) {
    ...
    if (!fileName.startsWith('flo-backup-') || !fileName.endsWith('.db'))
        return false;
    try {
...
function getSchemaDefinitions(dbInstance) {
...
function getColumns(dbInstance, tableName) {
    try {
...
function validateDirectBackup(backupPath, currentDb, currentVersion, baselineForeignKeyViolations = new Set()) {
    let backupDb;
    try {
        ...
        backupDb = new better_sqlite3_1.default(backupPath, { readonly: true, fileMustExist: true });
        const metaRow = backupDb.prepare(`SELECT value FROM _flo_meta WHERE key = 'schema_version'`).get();
...
function captureKitchenStationSecurityState(dbInstance) {
    try {
        return dbInstance.prepare('SELECT id, is_active, category_ids FROM kitchen_stations').all();
    }
    catch {
        return [];
    }
...
function captureRestoreProtectedSettings(dbInstance) {
    const fixedRows = dbInstance.prepare(`SELECT key, value FROM settings WHERE key IN (${RESTORE_PROTECTED_SETTING_KEYS.map(() => '?').join(',')})`).all(...RESTORE_PROTECTED_SETTING_KEYS);
    const cloudRows = dbInstance.prepare("SELECT key, value FROM settings WHERE key LIKE 'cloud_%'").all();
...
function mergeRestoreProtectedSettings(dbInstance, states) {
  ...
  `);
    ...
    for (const state of states) {
        if (state.present)
            upsert.run(state.key, state.value, now());
    ...
    if (!hasDeviceSecret || !hasPosHash)
        ensureCloudIdentity();
}
function captureRestoreOutboxState(dbInstance) {
    const pending = (table) => dbInstance.prepare(`SELECT * FROM ${table} WHERE status IN ('pending', 'failed', 'sending')`).all();
...
function mergeRestoreOutboxState(dbInstance, state) {
    ...
    for (const row of state.cloud)
        cloud.run(row.id, row.event_type, row.entity_type, row.entity_id, row.payload, row.status === 'sending' ? 'failed' : row.status, row.attempt_count || 0, row.next_attempt_at || now(), row.last_error || null, row.delivered_at || null, row.created_at || now(), row.updated_at || now());
    ...
    for (const row of state.support)
        support.run(row.client_ticket_id, row.payload, row.status === 'sending' ? 'failed' : row.status, row.support_code || null, row.attempt_count || 0, row.next_attempt_at || now(), row.last_error || null, row.created_at || now(), row.updated_at || now(), row.delivered_at || null);
    ...
    for (const row of state.diagnostics)
        diagnostics.run(row.event_id, row.payload, row.status === 'sending' ? 'failed' : row.status, row.attempt_count || 0, row.next_attempt_at || now(), row.last_error || null, row.created_at || now(), row.updated_at || now(), row.delivered_at || null);
}
function captureKdsEnabledSetting(dbInstance) {
    const row = dbInstance.prepare('SELECT value FROM settings WHERE key = ?').get('kds_enabled');
...
function mergeKdsEnabledSetting(dbInstance, state) {
    if (!state.present)
        return;
    dbInstance.prepare(`
...
function captureUserStationSecurityState(dbInstance) {
    try {
...
function mergeUserStationSecurityState(dbInstance, rows, userIds, preservedStations = []) {
    ...
    const restoreStationSecurity = dbInstance.prepare('UPDATE kitchen_stations SET is_active = ?, category_ids = ?, updated_at = ? WHERE id = ?');
    ...
    const invalidStations = rows
        ...
        .filter((row) => {
        const restored = stationState.get(row.station_id);
        return !restored
    ...
    const currentUsers = dbInstance.prepare('SELECT id FROM users').all();
    ...
    for (const row of rows) {
        if (preservedIds.has(row.user_id))
            insert.run(row.user_id, row.station_id, now());
    }
}
function getUserKdsStationIds(dbInstance, userId) {
    try {
        return dbInstance.prepare(`
    ...
    `).all(userId).map((row) => String(row.station_id));
    }
    catch {
        return null;
    }
}
function getKdsStationCategoryIds(dbInstance, stationIds) {
    if (stationIds.length === 0)
        return [];
    try {
        const placeholders = stationIds.map(() => '?').join(',');
        const rows = dbInstance.prepare(`SELECT category_ids FROM kitchen_stations WHERE is_active = 1 AND id IN (${placeholders})`).all(...stationIds);
...
function getKdsStationRoutingScope(dbInstance, stationIds, userCategoryIds) {
    if (stationIds.length === 0)
    ...
    `).all(...stationIds);
        ...
        for (const stationId of stationIds) {
            ...
            if (allowed === null)
                hasUnrestrictedStation = true;
            if (allowed !== null)
                allowed.forEach((id) => tableless.add(id));
        }
...
function getKdsStationRoutingCategoryIds(dbInstance, stationIds, userCategoryIds) {
    return getKdsStationRoutingScope(dbInstance, stationIds, userCategoryIds)?.tablelessCategoryIds ?? null;
}
function isKdsStationItemAllowed(stationIds, stationCategoryIds, orderStationId, itemCategoryId, orderStationCategoryIds, hasUnrestrictedStation = false) {
    if (stationIds.length === 0)
        return true;
    if (orderStationId) {
        if (!stationIds.includes(String(orderStationId)))
            return false;
        if (orderStationCategoryIds === undefined)
            return true;
        if (orderStationCategoryIds === null)
            return true;
...
function hasUserKdsStationAssignments(dbInstance, userId) {
    try {
...
function captureUserSecurityState(dbInstance) {
    try {
        return dbInstance.prepare('SELECT id, name, email, password, pin, pin_hash, role, category_ids, is_active, tokens_valid_after, station_assignments_configured FROM users').all();
    }
    catch {
        return [];
    }
}
function mergeUserSecurityState(dbInstance, rows) {
    for (const row of rows) {
        const restored = dbInstance.prepare('SELECT id, is_active, tokens_valid_after, station_assignments_configured FROM users WHERE id = ?').get(row.id);
        if (!restored)
            continue;
      ...
      WHERE id = ?
    `).run(row.name, row.email, row.password, row.pin, row.pin_hash, row.role, row.category_ids, row.is_active, tokensValidAfter, row.station_assignments_configured || 0, row.id);
    }
    ...
    const restoredUsers = dbInstance.prepare('SELECT id FROM users').all();
    ...
    for (const user of restoredUsers) {
        if (!preservedIds.has(user.id))
            disableRestoredOnly.run(now(), user.id);
    }
  ...
  `);
    for (const row of rows) {
        ...
        const emailConflict = row.email
            ? dbInstance.prepare('SELECT id FROM users WHERE email = ?').get(row.email)
            : undefined;
        if (emailConflict)
            dbInstance.prepare('UPDATE users SET email = NULL WHERE id = ?').run(emailConflict.id);
        insertPreservedUser.run(row.id, row.name, row.email, row.password, row.pin, row.pin_hash, row.role, row.category_ids, row.is_active, row.tokens_valid_after, row.station_assignments_configured || 0, now(), now());
    }
}
function readRevocations(dbInstance) {
    try {
        return dbInstance.prepare('SELECT token_hash, expires_at, revoked_at FROM revoked_tokens').all();
    }
    catch {
        return [];
    }
}
function mergeRevocations(dbInstance, rows) {
    if (rows.length === 0)
        return;
  ...
  `);
    for (const row of rows)
        merge.run(row.token_hash, row.expires_at, row.revoked_at);
}
function restoreBackup(backupPath, forceDirect = false, signal) {
    ...
    if (forceDirect || backupSchemaVersion === currentVersion) {
        ...
        try {
            ...
            const integrity = freshDb.prepare('PRAGMA integrity_check').all();
            ...
            try {
                ...
                throw new Error(`Direct restore failed: ${error?.message || 'unknown error'}; ` +
                    `live database recovery failed: ${recoveryError?.message || 'unknown error'}`, { cause: recoveryError });
            }
            throw error;
        }
        finally {
...
function getForeignKeyViolationKeys(dbInstance) {
    const rows = dbInstance.prepare('PRAGMA foreign_key_check').all();
    const keys = new Set();
    for (const row of rows) {
        let identity = row.rowid;
        try {
            const tableInfo = dbInstance.prepare(`PRAGMA table_info("${row.table.replace(/"/g, '""')}")`).all();
            ...
            const foreignKey = dbInstance.prepare(`PRAGMA foreign_key_list("${row.table.replace(/"/g, '""')}")`).all()
            ...
            if (selectedColumns.length > 0 && row.rowid != null) {
                const values = dbInstance.prepare(`SELECT ${selectedColumns.join(', ')} FROM "${row.table.replace(/"/g, '""')}" WHERE rowid = ?`).get(row.rowid);
                if (values)
                    identity = {
...
function materializeRestoreSource(sourcePath, livePath) {
...
function dataOnlyRestore(backupPath, backupVersion, currentVersion, preservedRevocations = [], preservedUserSecurity = [], preservedUserStations = [], preservedStationSecurity = [], preservedKdsEnabled = { present: false, value: null }, preservedProtectedSettings = [], preservedOutboxes = { cloud: [], support: [], diagnostics: [] }, signal) {
    throwIfDatabaseMaintenanceAborted(signal);
    ...
    try {
        const attachedDatabases = currentDb.prepare('PRAGMA database_list').all();
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                throw new Error(`Restore committed but source cleanup failed: ${detachError?.message || 'unknown error'}; ` +
        ...
        if (cleanupFailure) {
            try {
                ...
                error = new Error(`${error?.message || 'Restore failed'}; cleanup failed: ${cleanupFailure instanceof Error ? cleanupFailure.message : 'unknown error'}; ` +
...
function getSchemaVersionFromBackup(backupPath) {
    let backupDb;
    try {
        backupDb = new better_sqlite3_1.default(backupPath, { readonly: true, fileMustExist: true });
        const metaRow = backupDb.prepare(`SELECT value FROM _flo_meta WHERE key = 'schema_version'`).get();
...
function getCurrentSchemaVersion() {
    return (db ?? getDatabase()).pragma('user_version', { simple: true });
}
/**
 ...
 */
function buildIdealSchemaDb() {
    const idealDb = new better_sqlite3_1.default(':memory:');
...
exports.MIGRATIONS = [
    ...
    },
    {
        ...
        up: () => {
            ...
            if (!userColumns.includes('pin'))
                return;
            const usersWithPin = db.prepare('SELECT id, pin FROM users WHERE pin IS NOT NULL').all();
    ...
    },
    {
        ...
        up: () => {
            ...
            const customers = db.prepare("SELECT id, phone, country_code FROM customers WHERE phone IS NOT NULL AND phone != ''").all();
      ...
      `).all();
            ...
            for (const group of dupes) {
                ...
                const allRows = db.prepare(`SELECT * FROM customers WHERE id IN (${ids.map(() => '?').join(',')})
           ORDER BY created_at ASC, id ASC`).all(...ids);
        ...
        `).run(winner.email, winner.address, winner.notes, winner.country_code, now(), winner.id);
                ...
                for (const table of fkTables) {
                    db.prepare(`UPDATE ${table} SET customer_id = ? WHERE customer_id IN (${losers.map(() => '?').join(',')})`)
                        .run(winner.id, ...losers.map((l) => l.id));
                }
      ...
      `);
            ...
            const nonE164 = db.prepare("SELECT COUNT(*) as cnt FROM customers WHERE phone IS NOT NULL AND phone != '' AND phone NOT LIKE '+%'").get();
    ...
    },
    {
      ...
      `);
            const rows = db.prepare(`SELECT id, addons, created_at FROM order_items WHERE addons IS NOT NULL AND addons != '' AND addons != 'null'`).all();
    ...
    },
    {
        ...
        up: () => {
            db.prepare(`INSERT OR IGNORE INTO settings (key, value, updated_at) VALUES ('kds_default_view', 'tabs', ?)`).run(now());
        },
    },
    {
        version: 27,
    ...
    },
    {
      ...
      `);
            for (const pack of bundled_1.BUNDLED_COUNTRY_PACKS) {
                const versionId = (0, bundled_1.bundledPackVersionId)(pack);
                const packJson = JSON.stringify(pack);
                const installedAt = now();
                const alreadyInstalled = db.prepare('SELECT 1 FROM country_pack_versions WHERE id = ?').get(versionId);
                insertPack.run(pack.id, pack.publisher, pack.country, pack.jurisdiction, versionId, installedAt, installedAt);
                insertVersion.run(versionId, pack.id, pack.version, pack.schemaVersion, JSON.stringify({
                    id: pack.id,
                ...
                }), packJson, sha256Hex(packJson), pack.effectiveFrom, pack.effectiveTo || null, pack.minFloVersion, pack.publishedAt, installedAt);
                for (const category of pack.categories) {
                    insertCategory.run(`${versionId}:category:${category.id}`, versionId, category.id, category.label, category.defaultBehavior || null, JSON.stringify(category), installedAt);
                }
                for (const rule of pack.rules) {
                    insertRule.run(`${versionId}:rule:${rule.id}`, versionId, rule.id, rule.label, rule.type, rule.rate || null, rule.amount || null, rule.appliesPer || null, JSON.stringify(rule.baseRuleIds || []), JSON.stringify(rule), installedAt);
                }
                if (!alreadyInstalled) {
                    db.prepare(`
          ...
          `).run(pack.id, versionId, JSON.stringify({ source: 'application_bundle', version: pack.version }), installedAt);
                }
            }
        },
    },
    ...
    },
    {
      ...
      `);
            ...
            for (const [table, column] of normalize) {
                ...
                db.prepare(`UPDATE ${table} SET ${column} = substr(REPLACE(${column}, 'T', ' '), 1, 19) WHERE ${column} LIKE '%T%'`).run();
            }
        },
    },
    {
    ...
    },
    {
        ...
        up: () => {
            ...
            const changedAt = now();
            const demoUsers = db.prepare(`SELECT id, password FROM users WHERE id IN ('user-demo-manager', 'user-demo-cashier', 'user-demo-chef')`).all();
                    ...
                    for (const payment of payments) {
                        if (payment && typeof payment.method === 'string' && typeof payment.transaction_id === 'string' && payment.transaction_id.trim() !== '') {
                            ...
                            if (previous && previous.billId !== row.id)
                                conflictInsert.run(payment.method, payment.transaction_id, row.id, previous.createdAt, detectedAt);
                            else
                                seenRefs.set(key, { billId: row.id, createdAt });
    ...
    },
    {
      ...
      `);
            ...
            const addRef = (method, transactionId, billId, createdAt) => {
                const existing = findRef.get(method, transactionId);
            ...
            const existingRefs = db.prepare('SELECT method, transaction_id, bill_id, created_at FROM payment_transaction_refs').all();
            for (const ref of existingRefs)
                addRef(ref.method, ref.transaction_id, ref.bill_id, ref.created_at);
            const rows = db.prepare('SELECT id, payment_details FROM bills WHERE payment_details IS NOT NULL ORDER BY id').all();
            for (const row of rows) {
                try {
                    const parsed = JSON.parse(row.payment_details);
                    const payments = Array.isArray(parsed) ? parsed : [parsed];
    ...
    },
    {
      ...
      `);
            for (const row of paymentRows)
                insertPayment.run(row.user_id || 'legacy', row.idempotency_key, row.bill_id, row.request_hash, row.response_json, row.created_at);
            const orderRows = db.prepare('SELECT idempotency_key, request_hash, response_json, created_at FROM order_idempotency').all();
            ...
            for (const row of orderRows) {
                ...
                try {
                    ...
                    if (response?.order?.user_id != null)
                        userId = String(response.order.user_id);
                }
                catch {
    ...
    },
    {
      ...
      `);
            for (const row of paymentRows)
                updatePayment.run(row.user_id, row.idempotency_key);
    ...
    },
    {
        ...
        up: () => {
            ...
            const copyIfPresent = (oldKey, newKey) => {
                const existing = db.prepare('SELECT value FROM settings WHERE key = ?').get(oldKey);
                if (existing) {
      ...
      `).get();
            if (legacyUpi) {
                db.prepare(`INSERT OR IGNORE INTO payment_methods (name, is_active, sort_order, created_at, updated_at) VALUES ('UPI', 1, 10, ?, ?)`)
                    .run(now(), now());
                const upiId = Number(db.prepare(`SELECT id FROM payment_methods WHERE name = 'UPI' COLLATE NOCASE`).get().id);
                const rows = db.prepare('SELECT id, payment_details FROM bills WHERE payment_details IS NOT NULL').all();
                ...
                for (const row of rows) {
                    ...
                    if (changed)
                        update.run(JSON.stringify(Array.isArray(parsed) ? lines : lines[0]), row.id);
                }
            }
        },
    },
    ...
    },
    {
      ...
      `);
            ...
            const current = db.prepare("SELECT value FROM settings WHERE key = 'bill_template'").get();
    ...
    },
    {
      ...
      `).all();
            let normalized = 0;
            for (const row of rows) {
                if (crypto.createHash('sha256').update(row.payload_json, 'utf8').digest('hex') !== row.checksum)
                    continue;
                const validation = (0, print_1.validateMerchantTemplateText)(row.payload_json);
                if (!validation.ok)
                    continue;
                const payloadJson = (0, print_1.serializeMerchantTemplatePayload)(validation.payload);
                let previousPayloadJson = row.previous_payload_json;
                if (previousPayloadJson !== null) {
                    const previousValidation = (0, print_1.validateMerchantTemplateText)(previousPayloadJson);
                    if (previousValidation.ok) {
                        previousPayloadJson = (0, print_1.serializeMerchantTemplatePayload)(previousValidation.payload);
                    }
                }
                if (payloadJson === row.payload_json && previousPayloadJson === row.previous_payload_json)
                    continue;
                db.prepare(`
          UPDATE merchant_print_templates
          SET payload_json = ?, previous_payload_json = ?, checksum = ?
          WHERE id = ?
        `).run(payloadJson, previousPayloadJson, crypto.createHash('sha256').update(payloadJson, 'utf8').digest('hex'), row.id);
                normalized++;
            }
    ...
    },
    {
        ...
        up: () => {
            ...
            if (!columns.includes('cash_drawer_pulse_enabled'))
                return;
    ...
    },
    {
        ...
        name: 'add_shifts',
        up: () => {
            // Shift open/close reconciliation (#279, minimal scope): a shift is a
            // time-window marker with an opening cash-count and an optional closing
            // count. Only one shift may be open at a time. Closing cash/notes are
            // set at close time; separate shift-scoped attribution of every
            // bill/order is deferred (bills/orders are not stamped with shift_id).
            db.exec(`
    ...
    {
        version: 82,
        name: 'add_orders_delivery_address',
        up: () => {
            // Delivery orders carry a free-text delivery address (Google Places
            // autocomplete on the POS). Stored per order so the invoice can
            // reference it; empty for non-delivery orders.
            const orderColumns = getColumns(db, 'orders');
    ...
    {
        version: 83,
        name: 'repair_held_orders_table',
        up: () => {
            // v17 ("add_held_orders_table") may not have run on all databases that
            // report user_version >= 17 (e.g. a DB that was restored/imported or
            // reached a higher version through a path that skipped v17). Re-run the
            // CREATE TABLE idempotently so both affected databases and any that
            // already have the table end up correct.
            db.exec(`
        CREATE TABLE IF NOT EXISTS held_orders (
          id TEXT PRIMARY KEY,
          table_id TEXT NOT NULL,
...
function syncBackupBeforeMigration(fromVersion, toVersion) {
    if (buildingIdealSchema)
        return;
    let targetPath = '';
    let completed = false;
    try {
        ...
        let backupDb;
        try {
            backupDb = new better_sqlite3_1.default(targetPath);
            backupDb.pragma('journal_mode = DELETE');
            backupDb.exec(`
        CREATE TABLE IF NOT EXISTS _flo_meta (
          key TEXT PRIMARY KEY,
      ...
      `);
            ...
            backupDb.prepare(`INSERT OR REPLACE INTO _flo_meta (key, value) VALUES (?, ?)`).run('app_version', getAppVersion());
        }
        finally {
            backupDb?.close();
        }
...
class SchemaVersionMismatchError extends Error {
    ...
    constructor(dbVersion, appVersion) {
        super(`Database schema (v${dbVersion}) is newer than this app version supports (v${appVersion}). ` +
...
function runMigrations() {
    ...
    const target = exports.MIGRATIONS.length > 0 ? exports.MIGRATIONS[exports.MIGRATIONS.length - 1].version : 0;
    if (current > target) {
...
function createSchema() {
    ...
    -- Roles: ${role_permissions_1.ROLE_KEYS.join(', ')}
...
function createCloudSyncSchema() {
    db.exec(`
...
function seedCloudSyncDefaults() {
    createCloudSyncSchema();
    const serverUrl = getSettingValue('cloud_server_url');
    if (!serverUrl)
        upsertSetting('cloud_server_url', DEFAULT_CLOUD_SERVER_URL);
...
function seedInstallDefaults() {
    const insert = (key, value) => db.prepare('INSERT OR IGNORE INTO settings (key, value) VALUES (?, ?)').run(key, value);
...
function generateShortId(table, length = 6) {
...
function getNextSequence(name, date) {
...
function dateStampInTimezone(timezone) {
    try {
...
function datePartsInTimezone(timezone) {
    try {
        const parts = new Intl.DateTimeFormat('en-CA', {
            timeZone: timezone,
            year: 'numeric',
...
function clampFinancialYearStart(monthValue, dayValue) {
...
function financialYearSegment(timezone, startMonth, startDay) {
...
function invoicePeriodSegment(period, timezone, startMonth, startDay) {
    const date = dateStampInTimezone(timezone);
    if (period === 'monthly')
        return date.slice(0, 6);
    if (period === 'financial_year')
        return financialYearSegment(timezone, startMonth, startDay);
    return date;
}
...
function sanitizedNumberPrefix(value, fallback) {
...
function generateBillNumber() {
    ...
    const resetPeriod = ['never', 'daily', 'monthly', 'financial_year'].includes(configuredPeriod)
        ? configuredPeriod
        : 'daily';
    const timezone = getSettingValue('timezone') || 'Asia/Kolkata';
    const fyStart = clampFinancialYearStart(getSettingValue('invoice_financial_year_start_month'), getSettingValue('invoice_financial_year_start_day'));
 ...
 */
function dayBoundsInTimezone(date, timezone) {
    const [y, m, d] = date.split('-').map(Number);
    const format = (instant) => instant.toISOString().replace('T', ' ').replace(/\..*$/, '');
    try {
        const toUtc = (localWallTime) => {
...
function verifyPin(storedHash, inputPin) {
...
exports.KDS_VOIDED_ITEM_VISIBILITY_MS = 15 * 60 * 1000;
/**
 ...
 */
function isVoidedItemKdsVisible(voidedAt) {
    if (!voidedAt)
        return true;
    return Date.now() - parseDbTimestamp(voidedAt).getTime() < exports.KDS_VOIDED_ITEM_VISIBILITY_MS;
}
/** Remove customer/payment/order-financial fields from category-scoped KDS payloads. */
function projectKdsOrder(order, restricted) {
    if (!restricted)
        return order;
    const allowedFields = [
...
function projectKdsItem(item, restricted) {
    if (!restricted)
        return item;
    const allowedFields = [
    ...
    if (Array.isArray(item.addons)) {
        projected.addons = item.addons.map((addon) => {
            const safeAddon = {};
            for (const field of ['id', 'name', 'quantity']) {
                if (field in addon)
                    safeAddon[field] = addon[field];
            }
            return safeAddon;
        });
    }
...
function projectKdsStation(station, restricted, userCategoryIds = []) {
    if (!restricted)
        return station;
    ...
    if (typeof projected.category_ids === 'string' && userCategoryIds.length > 0) {
        try {
            ...
            if (Array.isArray(parsed))
                projected.category_ids = JSON.stringify(parsed.filter((id) => userCategoryIds.includes(String(id))));
        }
        catch {
 ...
 */
function insertOrderItemAddons(dbInstance, orderItemId, addons, createdAt) {
 ...
 */
function attachEffectiveAddons(dbInstance, items) {
    if (items.length === 0)
        return items;
    const ids = items.map((item) => item.id);
    const placeholders = ids.map(() => '?').join(',');
    const rows = dbInstance.prepare(`SELECT * FROM order_item_addons WHERE order_item_id IN (${placeholders}) ORDER BY id`).all(...ids);
...
function parseRowJson(row) {
    ...
    if (Array.isArray(taxBreakdown) && taxBreakdown.length > 0 && Array.isArray(taxBreakdown[0])) {
        ...
        try {
            fractionDigits = (0, countries_1.getCurrencyFractionDigits)(getSettingValue('currency') || 'INR');
        }
        catch { }
        const merged = {};
