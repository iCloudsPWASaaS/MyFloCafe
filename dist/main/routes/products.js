...
exports.searchSerpApiImages = searchSerpApiImages;
...
// ── SerpAPI Google Images search (optional, opt-in) ─────────────────────
// Mirrors the merchant-side implementation: requires SERPAPI_KEY, injects the
// "SerpAPI" source tag, maps Google Images results, and degrades to an empty
// list when unconfigured, rate-limited, or offline.
...
async function searchSerpApiImages(query) {
    ...
    try {
        const response = await fetch(`${SERPAPI_ENDPOINT}?engine=google_images&q=${encodeURIComponent(query)}&api_key=${encodeURIComponent(apiKey)}`, { signal: controller.signal });
        if (!response.ok) {
            console.error('[API] SerpAPI:', response.status, (await response.text().catch(() => '')));
            return [];
        }
        const payload = await response.json();
        if (!Array.isArray(payload.images_results)) {
            return [];
        }
        return payload.images_results
            ...
            full: item.original ?? null,
            width: item.original_width ?? null,
            height: item.original_height ?? null,
            context_url: item.link ?? null,
        }))
            .filter((result) => Boolean(result.full));
    }
    catch (error) {
        console.error('[API] SerpAPI Error:', error?.name === 'AbortError' ? 'request timed out' : error?.message ?? error);
        return [];
    }
    finally {
 ...
 */
async function resolvePublicHostname(hostname, signal) {
    const directAddress = hostname.replace(/^\[|\]$/g, '');
    if (net.isIP(directAddress)) {
        if ((0, security_1.isBlockedSsrfTarget)(directAddress))
    ...
    for (const { address } of addresses) {
        if ((0, security_1.isBlockedSsrfTarget)(address)) {
            throw new Error('URL resolves to a disallowed address');
        }
    }
    return addresses[0].address;
}
function fetchPinnedHttps(rawUrl, resolvedAddress, signal) {
 ...
 */
function loadProductRelationsBatch(db, products) {
    ...
    if (categoryIds.length > 0) {
        ...
        const categoryRows = db.prepare(`SELECT * FROM categories WHERE id IN (${catPlaceholders})`).all(...categoryIds);
    ...
    const agpRows = db.prepare(`SELECT product_id, addon_group_id FROM addon_group_product WHERE product_id IN (${placeholders})`).all(...productIds);
    // Group addon_group_ids by product_id
    const addonGroupIdsByProduct = new Map();
    ...
    const allAddonGroupIds = [...new Set(agpRows.map((r) => r.addon_group_id))];
    const addonGroupMap = new Map();
    if (allAddonGroupIds.length > 0) {
        const agPlaceholders = allAddonGroupIds.map(() => '?').join(',');
        const addonGroups = db.prepare(`SELECT * FROM addon_groups WHERE is_active = 1 AND id IN (${agPlaceholders})`).all(...allAddonGroupIds);
    ...
    if (allAddonGroupIds.length > 0) {
        const agPlaceholders = allAddonGroupIds.map(() => '?').join(',');
        const addons = db.prepare(`SELECT * FROM addons WHERE is_active = 1 AND addon_group_id IN (${agPlaceholders})`).all(...allAddonGroupIds);
...
const VALID_SALE_UNITS = ['each', 'kg', 'g', 'lb'];
...
function serializeAddonGroup(group) {
    if (!group)
        return group;
    return {
        ...group,
...
function validateProductNumericFields(values, requirePrice) {
...
function normalizeSaleUnit(value) {
    return VALID_SALE_UNITS.includes(value) ? value : 'each';
}
function validateWeightedProductFields(values, current) {
    if (values.sale_unit !== undefined && !VALID_SALE_UNITS.includes(values.sale_unit)) {
    ...
    if (values.weight_precision !== undefined) {
        if (!Number.isSafeInteger(values.weight_precision) || values.weight_precision < 0 || values.weight_precision > 4) {
...
function validateTaxCategoryId(categoryId) {
    if (categoryId === null || categoryId === undefined || categoryId === '')
        return null;
    if (typeof categoryId !== 'string')
        return 'tax_category_id must be a string or null';
    ...
    if (!(0, tax_1.hasConfiguredTaxCategories)(pack, businessType)) {
...
function validateCategoryId(db, categoryId) {
    if (categoryId === null || categoryId === undefined || categoryId === '')
        return null;
    if (typeof categoryId !== 'string')
...
function validateAddonGroupIds(db, rawIds) {
    if (rawIds === undefined)
        return {};
    ...
    const activeRows = db.prepare(`SELECT id FROM addon_groups WHERE is_active = 1 AND id IN (${placeholders})`).all(...uniqueIds);
...
router.get('/', (req, res) => {
    try {
      ...
      WHERE p.deleted_at IS NULL`;
        ...
        const relations = loadProductRelationsBatch(db, products);
...
// ── GET /image-search — Google Images via SerpAPI ───────────────────────
// Optional network feature (offline-first invariant): returns an empty list
// when SERPAPI_KEY is unconfigured or the upstream call fails, so the
// products UI degrades to the normal upload flows without blocking sales.
// Must be defined before /:id to avoid the catch-all route matching it.
router.get('/image-search', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (0, async_handler_1.asyncHandler)(async (req, res) => {
    const query = typeof req.query.q === 'string' ? req.query.q.trim().slice(0, 500) : '';
    if (!query) {
        return res.status(400).json({ error: 'Query is required' });
    }
    const results = await searchSerpApiImages(query);
    res.json({ results });
}));
// ── GET /:id — single product with relations ───────────────────────────
router.get('/:id/image', (0, async_handler_1.asyncHandler)(async (req, res) => {
    // Image endpoint — must be defined BEFORE /:id to avoid route conflict
    try {
        const db = (0, db_1.getDatabase)();
        const row = db.prepare('SELECT image_url FROM products WHERE id = ? AND deleted_at IS NULL').get(req.params.id);
...
router.get('/:id', (req, res) => {
    try {
        ...
        const relations = loadProductRelationsBatch(db, [product]);
        const rel = relations.get(product.id) || { category: null, addon_groups: [] };
        res.json({ product: serializeProduct({ ...product, tags: parseTags(product.tags), category: rel.category, addon_groups: rel.addon_groups }) });
    }
    catch (error) {
        console.error("[API] Internal error:", error);
        res.status(500).json({ error: "Internal server error" });
...
router.post('/fetch-url', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (0, async_handler_1.asyncHandler)(async (req, res) => {
    try {
        const { url } = req.body;
        ...
        if (requestSignal?.aborted)
            ...
            requestSignal?.addEventListener('abort', abortForShutdown, { once: true });
...
router.post('/', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
        const { category_id, name, sku, barcode, description, price, cost_price, sale_unit, allow_fractional_quantity, weight_precision, tax_category_id, tax_behavior, track_inventory, stock_quantity, low_stock_threshold, is_active, image_url, sort_order, cb_percent, tags, addon_group_ids } = req.body;
        ...
        if (numericError)
            return res.status(400).json({ error: numericError });
        const weightedFieldError = validateWeightedProductFields(req.body);
        if (weightedFieldError)
            return res.status(400).json({ error: weightedFieldError });
        ...
        if (normalizedBarcode) {
            const clash = db.prepare('SELECT id FROM products WHERE barcode = ? AND deleted_at IS NULL').get(normalizedBarcode);
            if (clash) {
                return res.status(400).json({ error: 'Another product already uses this barcode' });
            }
        }
        const id = (0, db_1.generateShortId)('products');
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(id, normalizeNullableString(category_id), productName, normalizeNullableString(sku), normalizedBarcode, normalizeNullableString(description), price, cost_price || 0, normalizeSaleUnit(sale_unit), allow_fractional_quantity ? 1 : 0, weight_precision ?? 3, 'none', 0, normalizeNullableString(tax_category_id), tax_behavior || 'country_default', track_inventory ? 1 : 0, stock_quantity || 0, low_stock_threshold || 0, is_active !== false ? 1 : 0, normalizeNullableString(image_url), sort_order || 0, cb_percent !== undefined ? cb_percent : null, JSON.stringify(tags || []), (0, db_1.now)(), (0, db_1.now)());
...
router.put('/:id', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
        ...
        if (normalizedBarcode) {
            const clash = db.prepare('SELECT id FROM products WHERE barcode = ? AND deleted_at IS NULL AND id != ?').get(normalizedBarcode, req.params.id);
            if (clash) {
                return res.status(400).json({ error: 'Another product already uses this barcode' });
            }
        }
...
router.delete('/:id', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
        ...
        db.prepare('UPDATE products SET deleted_at = ? WHERE id = ?').run((0, db_1.now)(), req.params.id);
...
router.post('/:id/stock', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
        ...
        if (action === 'set') {
            result = db.prepare('UPDATE products SET stock_quantity = ?, updated_at = ? WHERE id = ? AND deleted_at IS NULL')
                .run(quantity, (0, db_1.now)(), req.params.id);
        }
            ...
            result = db.prepare('UPDATE products SET stock_quantity = stock_quantity - ?, updated_at = ? WHERE id = ? AND deleted_at IS NULL AND stock_quantity >= ?')
                .run(quantity, (0, db_1.now)(), req.params.id, quantity);
        }
        if (result.changes === 0) {
...
router.get('/loyalty/global-rate-candidates', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (_req, res) => {
    try {
        const row = (0, db_1.getDatabase)().prepare('SELECT COUNT(*) AS count FROM products WHERE cb_percent = 0 AND deleted_at IS NULL').get();
...
router.post('/loyalty/apply-global-rate', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (_req, res) => {
    try {
        const result = (0, db_1.getDatabase)().prepare('UPDATE products SET cb_percent = NULL, updated_at = ? WHERE cb_percent = 0 AND deleted_at IS NULL').run((0, db_1.now)());
