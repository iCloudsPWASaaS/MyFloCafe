...
exports.addonGroupRoutes = void 0;
...
const addonGroupReadRateLimit = (0, express_rate_limit_1.default)({ windowMs: 60 * 1000, limit: 120, standardHeaders: true, legacyHeaders: false });
const addonGroupWriteRateLimit = (0, express_rate_limit_1.default)({ windowMs: 60 * 1000, limit: 60, standardHeaders: true, legacyHeaders: false });
...
function parseNonNegativeNumber(value, field, errors, defaultValue) {
...
function parseNonNegativeInteger(value, field, errors, defaultValue) {
    if (value === undefined || value === null || value === '')
        return defaultValue;
    const numeric = Number(value);
...
function invalidTaxBehavior(addons) {
...
function invalidTaxCategory(addons) {
    if (!Array.isArray(addons))
        return null;
...
function validateSelectionBounds(minSelection, maxSelection, activeAddonCount) {
...
function wouldBreakMinSelection(db, groupId, excludeAddonId) {
    const group = db.prepare('SELECT * FROM addon_groups WHERE id = ?').get(groupId);
    if (!group)
        return null;
    const remaining = db.prepare('SELECT COUNT(*) as count FROM addons WHERE addon_group_id = ? AND is_active = 1 AND id != ?').get(groupId, excludeAddonId).count;
...
router.get('/:id', addonGroupReadRateLimit, (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        ...
        res.json({ addon_group: serializeAddonGroup({ ...group, addons }) });
    }
    catch (error) {
        console.error("[API] Internal error:", error);
        res.status(500).json({ error: "Internal server error" });
...
router.post('/', addonGroupWriteRateLimit, (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
        ...
        const activeAddonCount = normalizedAddons.filter((a) => a.is_active).length;
        ...
        const { group, groupAddons } = (0, db_1.withTxn)(() => {
            db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(groupId, groupName, description || null, normalizeBoolean(is_required, false) ? 1 : 0, min, max, normalizeBoolean(allow_multiple_quantities, false) ? 1 : 0, order, (0, db_1.now)(), (0, db_1.now)());
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `);
                normalizedAddons.forEach((addon, index) => {
                    insertAddon.run((0, crypto_1.randomUUID)(), groupId, addon.name, addon.price || 0, addon.tax_category_id || null, addon.tax_behavior || 'country_default', addon.inherit_parent_tax_category !== false ? 1 : 0, addon.is_active ? 1 : 0, index, (0, db_1.now)(), (0, db_1.now)());
                });
            }
            return {
...
router.put('/:id', addonGroupWriteRateLimit, (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        const group = db.prepare('SELECT * FROM addon_groups WHERE id = ?').get(req.params.id);
        if (!group) {
        ...
        const effectiveMin = parseNonNegativeInteger(min_selection, 'min_selection', errors, group.min_selection);
        const effectiveMax = parseNonNegativeInteger(max_selection, 'max_selection', errors, group.max_selection);
        const order = parseNonNegativeInteger(sort_order, 'sort_order', errors, group.sort_order);
        if (addons !== undefined && !Array.isArray(addons)) {
            errors.addons = ['addons must be an array'];
        }
        const existingAddonRows = db.prepare('SELECT * FROM addons WHERE addon_group_id = ?').all(req.params.id);
        ...
        const activeAddonCount = Array.isArray(addons)
            ? normalizedAddons.filter((a) => a.is_active).length
            : db.prepare('SELECT COUNT(*) as count FROM addons WHERE addon_group_id = ? AND is_active = 1').get(req.params.id).count;
        ...
        const { updated, updatedAddons } = (0, db_1.withTxn)(() => {
            db.prepare(`
        ...
        WHERE id = ?
      `).run(reqName, hasDescription ? 1 : 0, reqDesc, reqReq, reqMin, reqMax, reqAllowMult, reqSort, reqActive, (0, db_1.now)(), req.params.id);
            if (normalizedAddons) {
        ...
        `);
                normalizedAddons.forEach((addon, index) => {
                    const addonId = addon.id || (0, crypto_1.randomUUID)();
                    ...
                    const values = [
                        ...
                        addon.is_active ? 1 : 0, index, (0, db_1.now)(),
                    ];
                    if (addon.id) {
                        updateAddon.run(...values, addonId, req.params.id);
                    }
                    else {
                        insertAddon.run(addonId, req.params.id, ...values.slice(0, -1), (0, db_1.now)(), (0, db_1.now)());
                    }
                });
                for (const existingAddon of existingAddonRows) {
                    if (!touchedIds.has(existingAddon.id) && existingAddon.is_active) {
                        db.prepare('UPDATE addons SET is_active = 0, updated_at = ? WHERE id = ?').run((0, db_1.now)(), existingAddon.id);
                    }
                }
            }
            return {
...
router.delete('/:id', addonGroupWriteRateLimit, (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        const group = db.prepare('SELECT * FROM addon_groups WHERE id = ?').get(req.params.id);
        if (!group) {
        ...
        db.prepare('UPDATE addon_groups SET is_active = 0, updated_at = ? WHERE id = ?').run((0, db_1.now)(), req.params.id);
...
router.post('/:groupId/addons', addonGroupWriteRateLimit, (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
        ...
        if (addonActive) {
            const activeAddonCount = db.prepare('SELECT COUNT(*) as count FROM addons WHERE addon_group_id = ? AND is_active = 1').get(req.params.groupId).count + 1;
            const boundsError = validateSelectionBounds(group.min_selection, group.max_selection, activeAddonCount);
            if (boundsError) {
                return res.status(400).json({ errors: boundsError });
            }
        }
        const addonId = (0, crypto_1.randomUUID)();
        db.prepare(`
      INSERT INTO addons (id, addon_group_id, name, price, tax_category_id, tax_behavior, inherit_parent_tax_category, is_active, sort_order, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(addonId, req.params.groupId, addonName, addonPrice, tax_category_id || null, tax_behavior || 'country_default', inherit_parent_tax_category !== false ? 1 : 0, addonActive ? 1 : 0, addonSort, (0, db_1.now)(), (0, db_1.now)());
...
router.put('/:groupId/addons/:addonId', addonGroupWriteRateLimit, (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
        const { name, price, tax_category_id, tax_behavior, inherit_parent_tax_category, is_active, sort_order } = req.body;
        const errors = {};
        ...
        WHERE id = ?
      `).run(addonName, addonPrice, hasTaxCategoryId ? 1 : 0, tax_category_id, tax_behavior, inherit_parent_tax_category === undefined ? null : (inherit_parent_tax_category ? 1 : 0), addonActive === null ? null : (addonActive ? 1 : 0), addonSort, req.params.addonId);
            return true;
        });
        if (updatedAtomically !== true) {
            if (updatedAtomically === false)
                return res.status(404).json({ error: 'Addon not found' });
...
router.delete('/:groupId/addons/:addonId', addonGroupWriteRateLimit, (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        const group = db.prepare('SELECT * FROM addon_groups WHERE id = ?').get(req.params.groupId);
        if (!group || !group.is_active) {
        ...
        const deletedAtomically = (0, db_1.withTxn)(() => {
            const current = db.prepare('SELECT is_active FROM addons WHERE id = ? AND addon_group_id = ?')
                .get(req.params.addonId, req.params.groupId);
            if (!current)
                return false;
            ...
            db.prepare('UPDATE addons SET is_active = 0, updated_at = ? WHERE id = ?').run((0, db_1.now)(), req.params.addonId);
            return true;
        });
        if (deletedAtomically !== true) {
            if (deletedAtomically === false)
                return res.status(404).json({ error: 'Addon not found' });
...
exports.addonGroupRoutes = router;
//# sourceMappingURL=addon-groups.js.map