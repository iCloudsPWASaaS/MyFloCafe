...
function tenantTimezone() {
    return (0, db_1.getSettingValue)('timezone') || 'Asia/Kolkata';
}
function reportToday() {
    return (0, db_1.localDateInTimezone)(new Date(), tenantTimezone());
}
function reportDayBounds(date) {
    return (0, db_1.dayBoundsInTimezone)(date, tenantTimezone());
}
function reportDate(value, fallback) {
    return typeof value === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(value) ? value : fallback;
}
function userNameById(db, ids) {
    const unique = [...new Set(ids.filter((id) => !!id))];
    if (!unique.length)
        return {};
    const rows = db.prepare(`SELECT id, name FROM users WHERE id IN (${unique.map(() => '?').join(',')})`).all(...unique);
 ...
 */
function bucketByLocalHourAndWeekday(timestamps, timeZone) {
    ...
    for (const ts of timestamps) {
        const d = (0, db_1.parseDbTimestamp)(ts);
        if (isNaN(d.getTime()))
            continue;
 ...
 */
function paymentMethodBreakdown(db, startDate, endDate = startDate, paidOnly = false, attributeRefundsToBillDate = false) {
...
function pickExtreme(counts, mode, include) {
    let best = null;
...
router.get('/daily-stats', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
    ...
    `).get(start, end, minorFactor, start, end);
        const paymentMethodsToday = paymentMethodBreakdown(db, today);
...
router.get('/summary', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        const minorFactor = (0, countries_1.getCurrencyMinorUnitFactor)((0, refund_1.getTenantCurrency)(db));
    ...
    `).get(start, end, minorFactor, start, end, start, end);
...
router.get('/financial-summary', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.owner), (req, res) => {
    try {
        const today = reportToday();
...
// GET /z-report — daily close snapshot documented in docs/API.md. Mirrors
// /financial-summary's attribution semantics but scoped to a single
// tenant-local day: "rung up" sales (non-cancelled bills raised that day) on
// one side, actual money movements (paid_at on that day) on the other.
router.get('/z-report', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.owner), (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        const minorFactor = (0, countries_1.getCurrencyMinorUnitFactor)((0, refund_1.getTenantCurrency)(db));
        const today = reportToday();
        ...
        const [start, end] = reportDayBounds(date);
        // Rung up: bills raised during the day, excluding cancelled orders.
        const rungUp = db.prepare(`
    ...
    `).get(start, end);
        // Money actually in/out during the day (refunds attributed to the bill's
        // payment date, matching /financial-summary).
        const collections = db.prepare(`
      SELECT COUNT(*) AS count, COALESCE(SUM(paid_amount), 0) AS gross_collected
      FROM bills WHERE paid_at >= ? AND paid_at < ?
    `).get(start, end);
        const refundTotals = db.prepare(`
      SELECT COUNT(*) AS count, COALESCE(SUM(CAST(r.amount_cents AS REAL)) / ?, 0) AS refunded
      FROM refunds r JOIN bills b ON b.id = r.bill_id
      WHERE b.paid_at >= ? AND b.paid_at < ?
    `).get(minorFactor, start, end);
    ...
    `).all(start, end);
      ...
      SELECT COUNT(*) AS count FROM customers WHERE created_at >= ? AND created_at < ?
    `).get(start, end);
        // Tax components, same basis as /tax-components so a printed Z matches the
        // dashboard's tax report row for row.
    ...
    `).all(start, end);
        const billOrders = (0, bills_1.getOrdersWithItemsForBills)(db, dayBills);
        const documents = dayBills.map((bill) => ({
            tax_amount: bill.tax_amount,
            tax_snapshot: bill.tax_snapshot,
            tax_breakdown: bill.tax_breakdown,
            items: billOrders.get(Number(bill.id))?.items || [],
        }));
        const grossCollected = Number(collections.gross_collected || 0);
        const refunded = Number(refundTotals.refunded || 0);
        // Shifts that opened during the tenant-local day, with their drawer
        // expectation (opening cash + net cash collections in the shift window)
        // and the counted-vs-expected variance for closed shifts.
        const dayShifts = db.prepare(`
      SELECT * FROM shifts WHERE opened_at >= ? AND opened_at < ? ORDER BY opened_at, id
    `).all(start, end);
        const names = userNameById(db, dayShifts.flatMap((s) => [s.opened_by, s.closed_by]));
        const shifts = dayShifts.map((s) => {
            const openingCash = Number(s.opening_cash || 0);
            const expectedCash = openingCash + (0, shifts_1.computeShiftNetCollections)(db, s);
            const closed = s.status === 'closed' && !!s.closed_at;
            const closingCash = closed ? Number(s.closing_cash || 0) : null;
            return {
                id: s.id,
                opened_at: s.opened_at,
                opened_by: s.opened_by,
                opened_by_name: s.opened_by ? names[s.opened_by] || null : null,
                closed_at: s.closed_at || null,
                closed_by: s.closed_by || null,
                closed_by_name: s.closed_by ? names[s.closed_by] || null : null,
                opening_cash: openingCash,
                closing_cash: closingCash,
                expected_cash: expectedCash,
                variance: closed ? Number(closingCash) - expectedCash : null,
                notes: s.notes || null,
                status: s.status,
            };
        });
        const closedShifts = shifts.filter((s) => s.variance !== null);
        const sum = (rows, key) => rows.reduce((acc, s) => acc + Number(s[key] || 0), 0);
        res.json({
            zReport: {
                date,
                ...
                counts: {
                    orders: orders.count,
                    bills: Number(rungUp.count || 0),
                    paidBills: Number(collections.count || 0),
                    refunds: Number(refundTotals.count || 0),
                    newCustomers: Number(newCustomers.count || 0),
                },
                sales: {
                    grossSales: Number(rungUp.total || 0),
                    discounts: Number(rungUp.discounts || 0),
                    taxAmount: Number(rungUp.tax || 0),
                    grossCollected,
                    refunded,
                    netCollected: grossCollected - refunded,
                    averageOrderValue: collections.count ? (grossCollected - refunded) / collections.count : 0,
                },
                paymentMethods: paymentMethodBreakdown(db, date, date, true, true),
                ordersByStatus,
                orderTypes,
                taxComponents: (0, tax_components_1.aggregateTaxComponents)(documents),
                shifts,
                shiftTotals: {
                    count: shifts.length,
                    open_count: shifts.length - closedShifts.length,
                    closed_count: closedShifts.length,
                    opening_cash: sum(shifts, 'opening_cash'),
                    closing_cash: sum(closedShifts, 'closing_cash'),
                    expected_cash: sum(closedShifts, 'expected_cash'),
                    variance: sum(closedShifts, 'variance'),
                },
            },
        });
    }
    catch (error) {
        console.error('[API] Z-report generation failed:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
...
router.get('/tax-components', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        const today = reportToday();
        const startDate = reportDate(req.query.start_date, today);
    ...
    `).all(windowStart, windowEnd);
        const orders = (0, bills_1.getOrdersWithItemsForBills)(db, bills);
        ...
        const taxAmount = bills.reduce((sum, bill) => sum.plus(bill.tax_amount || 0), new decimal_js_1.default(0));
        res.json({
            taxComponents: {
                startDate,
                endDate,
                ...
                components: (0, tax_components_1.aggregateTaxComponents)(documents),
            },
        });
    }
    catch (error) {
...
router.get('/sales', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        const today = reportToday();
        const startDate = reportDate(req.query.start_date, today);
    ...
    `).all(windowStart, windowEnd);
        ...
        for (const row of dailyRows) {
            const date = (0, db_1.localDateInTimezone)((0, db_1.parseDbTimestamp)(row.created_at), timeZone);
        ...
        const byPaymentMethod = paymentMethodBreakdown(db, startDate, endDate, true);
...
router.get('/topProducts', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        const today = reportToday();
        const startDate = reportDate(req.query.start_date, today);
...
router.get('/recentOrders', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        const requestedLimit = Number(req.query.limit);
...
router.get('/tables', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
...
router.get('/insights', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        const minorFactor = (0, countries_1.getCurrencyMinorUnitFactor)((0, refund_1.getTenantCurrency)(db));
        ...
        const timeZone = tenantTimezone();
        const today = (0, db_1.localDateInTimezone)(new Date(), timeZone);
        ...
        const [windowStart] = (0, db_1.dayBoundsInTimezone)(startDate, timeZone);
      ...
      FROM bills
      WHERE paid_at >= ?
    `).get(minorFactor, windowStart, windowStart);
    ...
    `).all(windowStart);
        ...
        const orderTimestamps = db.prepare(`SELECT created_at FROM orders WHERE created_at >= ? AND status != 'cancelled'`).all(windowStart).map((r) => r.created_at);
