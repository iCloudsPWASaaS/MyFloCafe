...
exports.taxPackRoutes = exports.LEGACY_TRUSTED_PACK_DIGESTS = void 0;
exports.validationChecklist = validationChecklist;
exports.installCatalogEntry = installCatalogEntry;
exports.reinstallPackVersion = reinstallPackVersion;
exports.slugifyTaxId = slugifyTaxId;
...
const trusted_signing_key_1 = require("../tax-packs/trusted-signing-key");
const async_handler_1 = require("../middleware/async-handler");
const shutdown_1 = require("../shutdown");
const countries_1 = require("../countries");
const router = (0, express_1.Router)();
const BUNDLED_PACKS_BY_ID = new Map(bundled_1.BUNDLED_COUNTRY_PACKS.map((pack) => [pack.id, pack]));
...
exports.LEGACY_TRUSTED_PACK_DIGESTS = {
...
const ENTITY_TYPES = ['product', 'addon', 'packaging', 'delivery', 'service_charge'];
const TAX_BEHAVIORS = ['country_default', 'inclusive', 'exclusive', 'exempt'];
...
class DisclaimerRequiredError extends Error {
    ...
    constructor(packId, version) {
        super('Community tax pack requires disclaimer acknowledgment before activation');
        this.packId = packId;
        this.version = version;
    }
}
function activateInstalledPack(pack, version, actorId, options = {}) {
    ...
    const definition = JSON.parse(version.pack_json);
    ...
    (0, db_1.withTxn)(() => {
        db.prepare(`UPDATE country_packs SET status = 'installed', updated_at = ? WHERE country = ? AND id != ?`)
            .run((0, db_1.now)(), pack.country, pack.id);
        if (previousVersionId) {
            ...
            db.prepare(`UPDATE tax_overrides SET pack_version_id = ?, updated_at = ? WHERE pack_version_id = ?`)
                .run(version.id, (0, db_1.now)(), previousVersionId);
        }
...
function persistPackArtifacts(version, definition, installedAt, printTemplates = []) {
    const db = (0, db_1.getDatabase)();
    db.prepare(`
  ...
  `).run(version.id, version.pack_id, version.version, version.schema_version, version.manifest_json, version.pack_json, version.digest, version.signature, version.effective_from, version.effective_to, version.min_flo_version, version.published_at, installedAt);
...
function persistPackContent(version, definition, installedAt, printTemplates = []) {
    const db = (0, db_1.getDatabase)();
  ...
  `);
    for (const category of definition.categories) {
        insertCategory.run(`${version.id}:category:${category.id}`, version.id, category.id, category.label, category.defaultBehavior || null, JSON.stringify(category), installedAt);
    }
  ...
  `);
    for (const rule of definition.rules) {
        insertRule.run(`${version.id}:rule:${rule.id}`, version.id, rule.id, rule.label, rule.type, rule.rate || null, rule.amount || null, rule.appliesPer || null, JSON.stringify(rule.baseRuleIds || []), JSON.stringify(rule), installedAt);
    }
  ...
  `);
    for (const template of printTemplates) {
        insertTemplate.run(template.id, version.pack_id, version.id, template.country, template.jurisdiction, template.displayName, JSON.stringify(template.paperColumns.map((width) => `cols-${width}`)), JSON.stringify(template.renderer), JSON.stringify(template.templatePayload ?? template.payload), version.status === 'active' ? 'active' : 'installed', installedAt);
    }
}
function trustStatus(pack, overrideCount) {
...
function activePackForCountry(country) {
    const db = (0, db_1.getDatabase)();
  ...
  `).get(country, country);
    if (!row)
        throw Object.assign(new Error(`No active tax pack is installed for ${country}`), { statusCode: 409 });
    return {
        pack: {
            id: row.id,
        ...
        definition: JSON.parse(row.pack_json),
    };
}
function validateOverrideTarget(packVersionId, entityType, entityId, categoryId) {
    ...
    const db = (0, db_1.getDatabase)();
    const category = db.prepare('SELECT 1 FROM tax_categories WHERE pack_version_id = ? AND category_id = ?').get(packVersionId, categoryId);
    if (!category) {
    ...
    if (normalizedType === 'addon') {
        ...
        if (!addon)
            throw Object.assign(new Error('Add-on not found'), { statusCode: 404 });
    }
    return { entityType: normalizedType, entityId: normalizedEntityId, categoryId };
}
function assertOverrideTargetAvailable(packVersionId, entityType, entityId, excludingId) {
    const duplicate = (0, db_1.getDatabase)().prepare(`
...
function containsUnsafeData(value) {
    ...
    if (value && typeof value === 'object')
        return Object.values(value).some(containsUnsafeData);
    return false;
}
...
function activationVectorPasses(pack) {
    const primaryCategory = pack.categories[0];
    if (!primaryCategory)
        return false;
    const calculate = (customer) => tax_engine_1.TaxEngine.calculate({
        pack,
    ...
    const taxAmount = new decimal_js_1.default(intra.taxAmount);
    const payableTotal = new decimal_js_1.default(intra.payableTotal);
    if (!taxAmount.isFinite() || taxAmount.isNegative())
        return false;
    ...
    const expectedBeforeRounding = new decimal_js_1.default(100).plus(taxAmount);
    if (!new decimal_js_1.default(intra.totalBeforePayableRounding).eq(expectedBeforeRounding))
        return false;
    if (!payableTotal.eq(expectedBeforeRounding.plus(intra.payableRoundingAdjustment)))
        return false;
    const hasInterstateCondition = pack.rules.some((rule) => primaryCategory.ruleIds.includes(rule.id) && rule.conditions?.customerStateRelation);
...
function audit(action, actorId, packId, packVersionId, overrideId, details) {
  ...
  `).run(action, packId, packVersionId, overrideId, actorId, JSON.stringify(details), (0, db_1.now)());
}
function validationChecklist(version, publicKey = trusted_signing_key_1.TRUSTED_TAX_PACK_SIGNING_PUBLIC_KEY) {
    const checks = [];
    const add = (id, passed, message) => checks.push({ id, passed, message });
    let pack;
    try {
        pack = JSON.parse(version.pack_json);
    }
    catch {
    ...
    add(3, /^\d+\.\d+\.\d+$/.test(pack.version) && Number.isFinite(effectiveFrom)
        ...
        && pack.version === version.version && pack.effectiveFrom === version.effective_from, 'Valid, internally consistent version and effective-date range');
    add(4, semverAtLeast(APP_VERSION, pack.minFloVersion), `FloCafe ${APP_VERSION} satisfies minimum compatible version ${pack.minFloVersion}`);
    add(5, version.digest === (0, crypto_1.createHash)('sha256').update(signedArtifactJson).digest('hex'), 'Stored artifact digest matches');
    const bundledDefinition = BUNDLED_PACKS_BY_ID.get(pack.id);
    const legacyTrustedDigest = exports.LEGACY_TRUSTED_PACK_DIGESTS[pack.id];
    const trustedArtifact = pack.publisher === 'local'
        ...
        : Boolean((bundledDefinition && JSON.stringify(bundledDefinition) === JSON.stringify(pack))
            || (version.signature === null && legacyTrustedDigest
                && (0, crypto_1.createHash)('sha256').update(JSON.stringify(pack), 'utf8').digest('hex') === legacyTrustedDigest)
            || (version.signature && (0, catalog_1.verifyTaxPackSignature)(signedArtifactJson, version.signature, publicKey)));
    ...
    const requiredDefaults = ['packaging', 'delivery', 'service_charge', 'addon'];
    ...
    for (const rule of pack.rules) {
        try {
            const value = new decimal_js_1.default(rule.type === 'fixed' ? rule.amount || '' : rule.rate || '');
    ...
    try {
        const increment = new decimal_js_1.default(pack.payableRounding.increment);
    ...
    add(13, amountsValid && payableIncrementValid
        ...
        && pack.taxRounding.decimalPlaces >= 0 && pack.taxRounding.decimalPlaces <= 6, 'Rates, fixed amounts, precision, and payable increment are within bounds');
    ...
    for (const category of pack.categories) {
        const fixedTotal = pack.rules
            ...
            .reduce((total, rule) => total.plus(rule.amount || 0), new decimal_js_1.default(0));
        if (fixedTotal.gt(0)) {
            try {
                const result = tax_engine_1.TaxEngine.calculate({
                    pack,
                    country: pack.country === '*' ? 'ZZ' : pack.country,
                    jurisdiction: pack.jurisdiction,
                    businessType: 'restaurant',
                ...
                if (result.lines.some((line) => new decimal_js_1.default(line.taxableBase).isNegative()))
    ...
    const recognizedConditions = pack.rules.every((rule) => {
        ...
        if (!conditions)
            return true;
        const keysValid = Object.keys(conditions).every((key) => ['businessTypes', 'customerStateRelation', 'customerExempt'].includes(key));
    ...
    add(16, recognizedConditions
        && pack.categories.every((category) => !category.defaultBehavior || TAX_BEHAVIORS.includes(category.defaultBehavior)), 'Every tax behavior and jurisdiction selector is recognized');
    add(17, ['unit', 'line', 'document'].includes(pack.taxRounding.scope)
        ...
        && new decimal_js_1.default(pack.payableRounding.increment).gt(0), 'Tax and payable rounding policies are complete');
    ...
    const active = (0, db_1.getDatabase)().prepare('SELECT pack_json FROM country_pack_versions WHERE pack_id = ? AND status = ? LIMIT 1').get(version.pack_id, 'active');
    const activePack = active ? parseJson(active.pack_json, null) : null;
    ...
    const activeVersion = (0, db_1.getDatabase)().prepare('SELECT active_version_id FROM country_packs WHERE id = ?').get(version.pack_id);
    const overrideConflicts = activeVersion?.active_version_id ? (0, db_1.getDatabase)().prepare(`
    SELECT COUNT(*) AS count
    FROM tax_overrides
    WHERE pack_version_id = ?
      AND json_extract(value_json, '$.categoryId') NOT IN (${categoryIds.map(() => '?').join(',') || "''"})
  `).get(activeVersion.active_version_id, ...categoryIds) : { count: 0 };
    ...
    add(20, overrideConflicts.count === 0 || pack.publisher === 'local', 'Every current merchant override resolves against this version');
    ...
    if (pack.registrationNumberFormat) {
        ...
        if (registrationFormatValid && /\([^()]*[+*][^()]*\)[+*]/.test(pattern))
            registrationFormatValid = false;
    }
    ...
    add(26, pack.publisher !== 'local' || !pack.sourceType || pack.sourceType === 'official', 'A local/manual pack never declares a community sourceType');
    return { valid: checks.every((check) => check.passed), checks };
}
async function installCatalogEntry(entry, options) {
    const publicKey = options.publicKey || trusted_signing_key_1.TRUSTED_TAX_PACK_SIGNING_PUBLIC_KEY;
    const artifact = await (0, catalog_1.downloadAndVerifyTaxPack)(entry, options.fetchImpl || fetch, publicKey, options.signal);
    const db = (0, db_1.getDatabase)();
    const existingPack = db.prepare('SELECT * FROM country_packs WHERE id = ?')
        .get(artifact.pack.id);
    if (existingPack && (existingPack.publisher !== artifact.pack.publisher
    ...
    const duplicate = db.prepare('SELECT id FROM country_pack_versions WHERE pack_id = ? AND version = ?').get(artifact.pack.id, artifact.pack.version);
    if (duplicate) {
        throw Object.assign(new Error('This tax pack version is already installed'), { statusCode: 409 });
    }
    const installedAt = (0, db_1.now)();
    ...
    const version = {
        ...
        manifest_json: JSON.stringify(artifact.artifactJson === artifact.packJson
            ? entry
    ...
    (0, db_1.withTxn)(() => {
      ...
      `).run(artifact.pack.id, artifact.pack.publisher, artifact.pack.country, artifact.pack.jurisdiction, installedAt, installedAt);
        }
        else {
            db.prepare('UPDATE country_packs SET updated_at = ? WHERE id = ?')
                .run(installedAt, artifact.pack.id);
        }
        const versionExists = db.prepare('SELECT 1 FROM country_pack_versions WHERE pack_id = ? AND version = ?').get(artifact.pack.id, artifact.pack.version);
        if (versionExists) {
            throw Object.assign(new Error('This tax pack version is already installed'), { statusCode: 409 });
        }
...
async function reinstallPackVersion(packId, versionId, options) {
    const db = (0, db_1.getDatabase)();
    const pack = db.prepare('SELECT * FROM country_packs WHERE id = ?').get(packId);
    const version = db.prepare('SELECT * FROM country_pack_versions WHERE id = ? AND pack_id = ?').get(versionId, packId);
    if (!pack || !version) {
    ...
    const publicKey = options.publicKey || trusted_signing_key_1.TRUSTED_TAX_PACK_SIGNING_PUBLIC_KEY;
    const remote = await (0, catalog_1.fetchRemoteTaxPackCatalog)(fetchImpl, options.signal);
    const entry = remote.catalog.packs.find((candidate) => candidate.id === pack.id && candidate.version === version.version);
    if (!entry) {
        throw Object.assign(new Error('This tax plugin version is no longer available in the plugin catalog'), { statusCode: 404 });
    }
    const artifact = await (0, catalog_1.downloadAndVerifyTaxPack)(entry, fetchImpl, publicKey, options.signal);
...
router.get('/', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (_req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        const storeCountry = (0, db_1.getSettingValue)('country') || 'IN';
...
router.get('/audit', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
...
router.get('/catalog', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (0, async_handler_1.asyncHandler)(async (req, res) => {
    try {
        const remote = await (0, catalog_1.fetchRemoteTaxPackCatalog)(fetch, (0, shutdown_1.getHttpRequestSignal)(req));
        const installedRows = (0, db_1.getDatabase)().prepare('SELECT pack_id, version FROM country_pack_versions').all();
...
router.get('/updates', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (0, async_handler_1.asyncHandler)(async (req, res) => {
    try {
        const remote = await (0, catalog_1.fetchRemoteTaxPackCatalog)(fetch, (0, shutdown_1.getHttpRequestSignal)(req));
    ...
    `).all();
        ...
        res.json({ checked_at: (0, db_1.now)(), release_tag: remote.releaseTag, updates });
    }
    catch (error) {
...
router.post('/ensure-country', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (0, async_handler_1.asyncHandler)(async (req, res) => {
    try {
        const country = String(req.body?.country || (0, db_1.getSettingValue)('country') || '').toUpperCase();
        if (!/^[A-Z]{2}$/.test(country))
            return res.status(400).json({ error: 'Invalid country' });
        const db = (0, db_1.getDatabase)();
        let pack = db.prepare(`SELECT * FROM country_packs WHERE country = ? AND status IN ('active', 'installed') ORDER BY status = 'active' DESC, updated_at DESC LIMIT 1`).get(country);
        let version = pack?.active_version_id
            ? db.prepare('SELECT * FROM country_pack_versions WHERE id = ? AND pack_id = ?').get(pack.active_version_id, pack.id)
            : undefined;
        if (!version && pack) {
      ...
      `).get(pack.id);
        ...
        if (!version) {
            const requestSignal = (0, shutdown_1.getHttpRequestSignal)(req);
            const remote = await (0, catalog_1.fetchRemoteTaxPackCatalog)(fetch, requestSignal);
            ...
            if (!entry)
                return res.status(404).json({ plugin_available: false, country, error: `Tax support for ${country} is not available yet` });
            const installed = await installCatalogEntry(entry, { actorUserId: actorUserId(req), signal: requestSignal });
            pack = db.prepare('SELECT * FROM country_packs WHERE id = ?').get(installed.packId);
            version = db.prepare('SELECT * FROM country_pack_versions WHERE id = ?').get(installed.versionId);
        }
        if (!pack || !version)
            throw new Error('Installed tax pack could not be loaded');
        try {
        ...
        (0, db_1.withTxn)(() => {
            db.prepare(`UPDATE products SET tax_category_id = ?, updated_at = ? WHERE tax_category_id IS NULL AND deleted_at IS NULL`)
                .run(definition.defaultCategories.product, (0, db_1.now)());
            db.prepare(`UPDATE addons SET tax_category_id = ? WHERE tax_category_id IS NULL`)
                .run(definition.defaultCategories.addon);
        });
        (0, db_1.upsertSettings)({ taxes_enabled: 'true' });
        return res.json({
            enabled: true,
            country,
            pack_id: pack.id,
            version: version.version,
            tax_id_format: (0, tax_1.resolveTaxIdFormat)(country),
        });
    }
    catch (error) {
...
function slugifyTaxId(label, used, fallback) {
    ...
    let start = 0;
    while (start < base.length && base[start] === '_')
        start += 1;
    let end = base.length;
    while (end > start && base[end - 1] === '_')
        end -= 1;
...
function buildManualPack(body, country, currency) {
    ...
    const tempIdToCategoryId = new Map();
    const categories = [];
    const rules = [];
    categoriesInput.forEach((categoryInput, categoryIndex) => {
        ...
        const ruleIds = [];
        componentsInput.forEach((componentInput, componentIndex) => {
            const componentLabel = typeof componentInput?.label === 'string' && componentInput.label.trim()
                ? componentInput.label.trim() : label;
            const type = componentInput?.type === 'fixed' ? 'fixed' : 'percent';
            let value;
            try {
                value = new decimal_js_1.default(componentInput?.value === undefined || componentInput?.value === null ? '' : String(componentInput.value));
            }
            catch {
    ...
    const resolveDefault = (fieldName, tempId) => {
...
router.post('/manual-config', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.owner), (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        const country = String((0, db_1.getSettingValue)('country') || '').toUpperCase();
        if (!/^[A-Z]{2}$/.test(country)) {
            return res.status(400).json({ error: 'A valid store country must be set in Settings first' });
        }
        const currency = String((0, db_1.getSettingValue)('currency') || 'USD').toUpperCase();
        const activeForCountry = db.prepare(`SELECT id, publisher FROM country_packs WHERE country = ? AND status = 'active'`).get(country);
        ...
        const digest = (0, crypto_1.createHash)('sha256').update(packJson, 'utf8').digest('hex');
        const installedAt = (0, db_1.now)();
        ...
        let remapped = [];
        (0, db_1.withTxn)(() => {
            const existingPackRow = db.prepare('SELECT * FROM country_packs WHERE id = ?').get(pack.id);
            if (!existingPackRow) {
                db.prepare(`
        ...
        `).run(pack.id, country, installedAt, installedAt);
            ...
            const packRow = db.prepare('SELECT * FROM country_packs WHERE id = ?').get(pack.id);
            ...
            const staleProducts = db.prepare(`SELECT COUNT(*) AS total FROM products WHERE deleted_at IS NULL AND tax_category_id IS NOT NULL AND tax_category_id NOT IN (${placeholders})`).get(...categoryIds);
            const staleAddons = db.prepare(`SELECT COUNT(*) AS total FROM addons WHERE tax_category_id IS NOT NULL AND tax_category_id NOT IN (${placeholders})`).get(...categoryIds);
            db.prepare(`UPDATE products SET tax_category_id = ?, updated_at = ? WHERE deleted_at IS NULL AND (tax_category_id IS NULL OR tax_category_id NOT IN (${placeholders}))`).run(pack.defaultCategories.product, installedAt, ...categoryIds);
            db.prepare(`UPDATE addons SET tax_category_id = ? WHERE tax_category_id IS NULL OR tax_category_id NOT IN (${placeholders})`).run(pack.defaultCategories.addon, ...categoryIds);
            ...
            const overrideRows = db.prepare(`SELECT id, entity_type, value_json FROM tax_overrides WHERE pack_version_id = ? AND field_name = 'tax_category_id'`).all(versionId);
            const updateOverrideStmt = db.prepare(`UPDATE tax_overrides SET value_json = ?, updated_at = ? WHERE id = ?`);
            const overrideRemapCounts = new Map();
            for (const override of overrideRows) {
                const currentCategoryId = parseJson(override.value_json, {}).categoryId;
                if (currentCategoryId && categoryIds.includes(currentCategoryId))
                    continue;
...
router.post('/catalog/install', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.owner), (0, async_handler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        const requestSignal = (0, shutdown_1.getHttpRequestSignal)(req);
        const remote = await (0, catalog_1.fetchRemoteTaxPackCatalog)(fetch, requestSignal);
        const entry = remote.catalog.packs.find((candidate) => candidate.id === packId && candidate.version === version);
        if (!entry)
...
router.post('/test-calculation', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
        const { category_id, amount, tax_behavior } = req.body;
        const amountDecimal = new decimal_js_1.default(String(amount));
        ...
        const currency = (0, db_1.getSettingValue)('currency') || 'INR';
        ...
        const calculation = tax_engine_1.TaxEngine.calculate({
            pack: active.definition,
            currency,
            country,
            jurisdiction: active.definition.jurisdiction,
            businessType: (0, db_1.getSettingValue)('business_type') || 'restaurant',
            storeStateCode: (0, db_1.getSettingValue)('state_code') || '',
        ...
        const payableRounding = (0, tax_engine_1.applyPayableRounding)(Number(calculation.totalBeforePayableRounding), active.definition, currency);
        res.json({
            ...
            calculation: {
                ...
                taxableBase: calculation.lines
                    .reduce((sum, line) => sum.plus(line.taxableBase), new decimal_js_1.default(0))
                    .toFixed((0, countries_1.getCurrencyFractionDigits)(currency)),
            },
        });
    }
    catch (error) {
...
router.post('/overrides', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.owner), (req, res) => {
    try {
        const country = (0, db_1.getSettingValue)('country') || 'IN';
        const active = activePackForCountry(country);
        const target = validateOverrideTarget(active.version.id, req.body.entity_type, req.body.entity_id, req.body.category_id);
      ...
      `).run(id, active.version.id, target.entityType, target.entityId, JSON.stringify({ categoryId: target.categoryId }), actorUserId(req), (0, db_1.now)(), (0, db_1.now)());
        ...
        res.status(201).json({ override: (0, db_1.getDatabase)().prepare('SELECT * FROM tax_overrides WHERE id = ?').get(id) });
    }
    catch (error) {
        const statusCode = error.statusCode || 500;
        res.status(statusCode).json({
...
router.put('/overrides/:overrideId', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.owner), (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        const existing = db.prepare('SELECT * FROM tax_overrides WHERE id = ?').get(req.params.overrideId);
        if (!existing)
            return res.status(404).json({ error: 'Override not found' });
        const pack = db.prepare(`
      SELECT pack.* FROM country_packs AS pack
      WHERE pack.active_version_id = ?
    `).get(existing.pack_version_id);
        if (!pack)
            return res.status(409).json({ error: 'Override does not belong to an active pack version' });
        const currentValue = parseJson(existing.value_json, {});
        const target = validateOverrideTarget(existing.pack_version_id, req.body.entity_type ?? existing.entity_type, req.body.entity_id !== undefined ? req.body.entity_id : existing.entity_id, req.body.category_id ?? currentValue.categoryId);
        (0, db_1.withTxn)(() => {
            assertOverrideTargetAvailable(existing.pack_version_id, target.entityType, target.entityId, existing.id);
            db.prepare(`
        UPDATE tax_overrides
        SET entity_type = ?, entity_id = ?, value_json = ?, updated_at = ?
        WHERE id = ?
      `).run(target.entityType, target.entityId, JSON.stringify({ categoryId: target.categoryId }), (0, db_1.now)(), existing.id);
...
router.delete('/overrides/:overrideId', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.owner), (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
...
router.post('/:packId/versions/:versionId/activate', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.owner), (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        const pack = db.prepare('SELECT * FROM country_packs WHERE id = ?').get(req.params.packId);
        const version = db.prepare('SELECT * FROM country_pack_versions WHERE id = ? AND pack_id = ?').get(req.params.versionId, req.params.packId);
        if (!pack || !version)
...
router.post('/:packId/versions/:versionId/reinstall', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.owner), (0, async_handler_1.asyncHandler)(async (req, res) => {
    try {
        const requestSignal = (0, shutdown_1.getHttpRequestSignal)(req);
...
router.post('/:packId/rollback', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.owner), (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        const pack = db.prepare('SELECT * FROM country_packs WHERE id = ?').get(req.params.packId);
        if (!pack)
            return res.status(404).json({ error: 'Tax pack not found' });
        const target = db.prepare(`
      SELECT * FROM country_pack_versions
      WHERE pack_id = ? AND id != ? AND status NOT IN ('revoked', 'incompatible')
      ORDER BY published_at DESC, created_at DESC LIMIT 1
    `).get(pack.id, pack.active_version_id);
        if (!target)
            return res.status(400).json({ error: 'No previous installed version is available for rollback' });
...
router.get('/:packId', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.ownerManager), (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        const pack = db.prepare('SELECT * FROM country_packs WHERE id = ?').get(req.params.packId);
        if (!pack)
            return res.status(404).json({ error: 'Tax pack not found' });
        const versions = db.prepare('SELECT * FROM country_pack_versions WHERE pack_id = ? ORDER BY published_at DESC, version DESC').all(pack.id);
        const activeVersion = versions.find((version) => version.id === pack.active_version_id) || null;
        const categories = activeVersion ? db.prepare('SELECT category_id, label, default_behavior, definition_json FROM tax_categories WHERE pack_version_id = ? ORDER BY label').all(activeVersion.id).map((row) => ({ ...row, definition: parseJson(row.definition_json, {}) })) : [];
        const rules = activeVersion ? db.prepare(`SELECT rule_id, label, calculation_type, rate, amount, applies_per, base_rule_ids, definition_json
       FROM tax_rules WHERE pack_version_id = ? ORDER BY label`).all(activeVersion.id).map((row) => ({
            ...row,
    ...
    `).all(activeVersion.id).map((row) => ({
            ...row,
            value: parseJson(row.value_json, null),
        })) : [];
        const targets = {
            products: db.prepare(`SELECT id, name, tax_category_id FROM products WHERE deleted_at IS NULL AND is_active = 1 ORDER BY name`).all(),
            addons: db.prepare(`SELECT id, name, tax_category_id FROM addons WHERE is_active = 1 ORDER BY name`).all(),
        };
        res.json({
            pack: {
                ...pack,
...
exports.taxPackRoutes = router;
//# sourceMappingURL=tax-packs.js.map