...
function supportProfile(req) {
    ...
    const currentUser = userId
        ? db.prepare('SELECT name, email, role FROM users WHERE id = ? AND is_active = 1').get(userId)
        : null;
    const owner = db.prepare("SELECT name, email, role FROM users WHERE role = 'owner' AND is_active = 1 ORDER BY created_at ASC LIMIT 1").get();
...
function buildSystemDiagnostics(req, category) {
    const db = (0, db_1.getDatabase)();
    ...
    return {
        ...
        cloud: (() => {
            const status = cloud_sync_1.cloudSync.getStatus();
            return {
...
router.get('/profile', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.allStaff), (req, res) => {
    const profile = supportProfile(req);
    res.json({ ...profile, app_version: require('../../package.json').version, platform: process.platform });
});
router.get('/diagnostics-preview', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.allStaff), (req, res) => {
    res.json(buildSystemDiagnostics(req, resolveCategory(req.query.category)));
});
router.get('/:clientTicketId/status', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.allStaff), (req, res) => {
    ...
    const row = (0, db_1.getDatabase)().prepare('SELECT status, support_code, last_error FROM support_ticket_outbox WHERE client_ticket_id = ?').get(clientTicketId);
    if (!row)
        return res.status(404).json({ error: 'not found' });
    res.json({ status: row.status, support_code: row.support_code, last_error: row.last_error });
});
router.post('/', (0, security_1.requireRole)(...role_permissions_1.ROLE_ACCESS.allStaff), (0, async_handler_1.asyncHandler)(async (req, res) => {
    ...
    const clientTicketId = typeof body.client_ticket_id === 'string' && CLIENT_TICKET_ID_RE.test(body.client_ticket_id)
        ? body.client_ticket_id : (0, crypto_1.randomUUID)();
    ...
    if (body.contact_phone !== undefined && body.contact_phone !== null && String(body.contact_phone).trim() !== '') {
        const phoneRes = (0, phone_1.normalizeOptionalPhone)(body.contact_phone, profile.country || 'IN');
        ...
        const phoneRes = (0, phone_1.normalizeOptionalPhone)(profile.contact_phone, profile.country || 'IN');
    ...
    const queued = await cloud_sync_1.cloudSync.queueSupportTicket({
