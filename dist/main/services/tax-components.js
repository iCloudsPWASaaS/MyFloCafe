...
exports.resolveTaxComponents = resolveTaxComponents;
exports.aggregateTaxComponents = aggregateTaxComponents;
...
function hasSplitAllocatedSnapshot(value) {
    const parsed = parseJson(value);
    if (Array.isArray(parsed))
        return parsed.some(hasSplitAllocatedSnapshot);
...
function flattenSnapshots(value, onlyChargeSnapshots = false) {
    const parsed = parseJson(value);
    if (Array.isArray(parsed)) {
...
function flattenLegacyBreakdown(value) {
    const parsed = parseJson(value);
    if (!Array.isArray(parsed))
        return [];
    const components = [];
...
function mergeComponents(components) {
    ...
    return Array.from(merged.values()).map((component) => ({
        ...
        rate: component.rate === null ? null : new decimal_js_1.default(component.rate).toNumber(),
        amount: component.amount.toDecimalPlaces(6).toNumber(),
    }));
}
function reconcileTotal(components, targetValue) {
    ...
    const current = components.reduce((sum, component) => sum.plus(component.amount), new decimal_js_1.default(0));
    ...
    const reconciled = components.map((component) => ({
        ...component,
        amount: new decimal_js_1.default(component.amount).times(ratio).toDecimalPlaces(6).toNumber(),
    }));
    const allocated = reconciled.reduce((sum, component) => sum.plus(component.amount), new decimal_js_1.default(0));
    reconciled[reconciled.length - 1].amount = new decimal_js_1.default(reconciled[reconciled.length - 1].amount).plus(target.minus(allocated)).toDecimalPlaces(6).toNumber();
    return reconciled;
}
function legacyItemComponents(document) {
    return (document.items || []).filter((item) => item.status !== 'cancelled' && item.status !== 'voided' && item.status !== 'void_adjustment' && item.status !== 'refunded').flatMap((item) => {
        const snapshot = flattenSnapshots(item.tax_snapshot);
        return snapshot.present ? [] : flattenLegacyBreakdown(item.tax_breakdown);
    });
}
function documentLegacyComponents(document, coveredComponents = []) {
    const remainingItems = new Map();
    for (const component of [...legacyItemComponents(document), ...coveredComponents]) {
        const key = `${component.title}\u0000${component.rate ?? ''}`;
        remainingItems.set(key, (remainingItems.get(key) || new decimal_js_1.default(0)).plus(component.amount));
    }
    return flattenLegacyBreakdown(document.tax_breakdown).flatMap((component) => {
        const key = `${component.title}\u0000${component.rate ?? ''}`;
        const itemAmount = remainingItems.get(key) || new decimal_js_1.default(0);
        ...
        const consumed = sameSign
            ? decimal_js_1.default.min(itemAmount.abs(), component.amount.abs()).mul(component.amount.isNegative() ? -1 : 1)
 ...
 */
function resolveTaxComponents(document) {
    ...
    if (hasSplitAllocatedSnapshot(document.tax_snapshot)) {
        ...
        if (splitSnapshot.present) {
            ...
            const representedTotal = represented.reduce((sum, component) => sum.plus(component.amount), new decimal_js_1.default(0));
            const needsDocumentResidual = !target
                || (!target.isZero() && (target.isNegative()
                    ? representedTotal.comparedTo(target) > 0
                    : representedTotal.comparedTo(target) < 0));
            return reconcileTotal(mergeComponents([
                ...represented.map((component) => ({
                    title: component.title,
                    rate: component.rate === null ? null : new decimal_js_1.default(component.rate).toString(),
                    amount: new decimal_js_1.default(component.amount),
                })),
    ...
    const activeItems = document.items?.filter((item) => item.status !== 'cancelled' && item.status !== 'voided' && item.status !== 'void_adjustment' && item.status !== 'refunded');
    if (activeItems && activeItems.length > 0) {
        const components = [];
        const itemSnapshotComponents = [];
        ...
        for (const item of activeItems) {
            ...
            if (snapshot.present)
                itemSnapshotComponents.push(...snapshot.components);
        }
        if (hasItemTaxEvidence) {
            ...
            if (!hasSnapshotEvidence && !chargeSnapshot.present) {
                ...
                if (documentLegacy.length > 0)
                    return mergeComponents(documentLegacy);
            }
            if (chargeSnapshot.present) {
                const chargeComponents = mergeComponents(chargeSnapshot.components);
                const chargeTotal = chargeComponents.reduce((sum, component) => sum.plus(component.amount), new decimal_js_1.default(0));
    ...
    const components = mergeComponents(snapshot.present
        ? [
            ...snapshot.components,
            ...documentLegacyComponents(document, snapshot.components),
        ]
        : flattenLegacyBreakdown(document.tax_breakdown));
    return reconcileTotal(components, snapshot.present || components.length === 0 ? document.tax_amount : undefined);
}
function aggregateTaxComponents(documents) {
    const components = [];
...
//# sourceMappingURL=tax-components.js.map