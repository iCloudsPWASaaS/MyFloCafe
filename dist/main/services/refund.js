...
exports.TERMINAL_ITEM_STATUSES = void 0;
exports.getTenantCurrency = getTenantCurrency;
exports.getRefundableBalance = getRefundableBalance;
...
exports.TERMINAL_ITEM_STATUSES = ['cancelled', 'voided', 'void_adjustment', 'refunded'];
function getTenantCurrency(db) {
    const explicit = db ? db.prepare("SELECT value FROM settings WHERE key = 'currency'").get()?.value : (0, db_1.getSettingValue)('currency');
    if (explicit && typeof explicit === 'string' && /^[A-Z]{3}$/.test(explicit))
        return explicit;
    const country = (db ? db.prepare("SELECT value FROM settings WHERE key = 'country'").get()?.value : (0, db_1.getSettingValue)('country')) || 'IN';
 ...
 */
function getRefundableBalance(db, billId, currency) {
    const effectiveCurrency = currency || getTenantCurrency(db);
    const minorFactor = (0, countries_1.getCurrencyMinorUnitFactor)(effectiveCurrency);
    const bill = db.prepare('SELECT paid_amount FROM bills WHERE id = ?').get(billId);
    const paidCents = Math.round(Number(bill?.paid_amount || 0) * minorFactor);
    const refundedRow = db.prepare('SELECT COALESCE(SUM(amount_cents), 0) AS total FROM refunds WHERE bill_id = ?').get(billId);
    const refundedCents = Number(refundedRow.total || 0);
    return { paidCents, refundedCents, refundableCents: paidCents - refundedCents };
}
function resolveRefundApprover(db, overridePin, managerId) {
    if (managerId) {
        ...
        if (candidate && (0, db_1.verifyPin)(candidate.pin_hash, overridePin))
            return candidate;
    }
    ...
    for (const user of managers) {
        if ((0, db_1.verifyPin)(user.pin_hash, overridePin))
            return user;
    }
    return null;
}
...
function createRefund(db, req) {
    ...
    const bill = db.prepare('SELECT * FROM bills WHERE id = ?').get(req.billId);
    if (!bill)
        throw httpError('Bill not found', 404);
    const order = db.prepare('SELECT created_at FROM orders WHERE id = ?').get(bill.order_id);
    if (!order)
        throw httpError('Order not found', 404);
    const orderCreatedAt = (0, db_1.parseDbTimestamp)(order.created_at).getTime();
    ...
    if (req.orderItemId != null) {
        item = db.prepare('SELECT * FROM order_items WHERE id = ?').get(req.orderItemId);
        if (!item)
    ...
    if (refundableCents <= 0)
        throw httpError('Bill has nothing left to refund', 400);
    if (amountCents > refundableCents)
        throw httpError('Refund amount exceeds the refundable balance', 400);
    if (!req.overridePin) {
    ...
    if (!approver)
        throw httpError('Invalid manager PIN', 403);
    ...
    `).run(item.order_id, item.product_id, `Refund: ${item.product_name}`, item.product_sku, -item.unit_price, item.quantity, -item.subtotal, -(item.tax_amount || 0), (0, tax_1.invertTaxBreakdown)(item.tax_breakdown), (0, tax_1.invertTaxSnapshot)(item.tax_snapshot), item.tax_type, -(item.discount_amount || 0), -item.total, item.variant_selection, item.modifier_selection, timestamp, timestamp);
        if (bill.split_group_id) {
  ...
  `).run(req.billId, req.orderItemId ?? null, amountCents, req.method, req.reason ?? null, req.shiftId ?? null, approver.id, req.createdByUserId, timestamp);
    ...
    const result = { refund, bill: freshBill };
