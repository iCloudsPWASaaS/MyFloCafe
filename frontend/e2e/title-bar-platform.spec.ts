...
test('sidebar offset regression (#504): expanded/collapsed states at md+ viewports', async ({ page }) => {
  ...
  for (const viewport of VIEWPORTS) {
    await page.setViewportSize({ width: viewport.width, height: viewport.height });
    const routes = [
      // POS auto-collapses to the icon rail on entry so orders get max width.
    ...
    for (const route of routes) {
      ...
      if (route.startsCollapsed) {
        await expect(sidebarState, `${viewport.name} ${route.name}: POS auto-collapses to rail`).toHaveAttribute('data-state', 'collapsed');
        await expect(sidebarState, `${viewport.name} ${route.name}: POS collapsed uses icon mode`).toHaveAttribute('data-collapsible', 'icon');
