...
export function buildClassicReceiptBytes(
  ...
  if (customer?.address) {
    safePrinterText(enc, customer.address.text, warnings, false, arabicShaping, cols).newline();
  }
...
export function buildCompactReceiptBytes(
  ...
  if (customer?.address) {
    safePrinterText(enc, `${printLabelResolver('pos.deliveryAddress', primaryLang)}: ${customer.address.text}`, warnings, false, arabicShaping).newline();
  }
