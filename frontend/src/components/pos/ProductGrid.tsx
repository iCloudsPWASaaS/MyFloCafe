...
import { buildCategoryTree, getCategorySelectionIds } from '@/lib/categories';
      ...
      <div className="shrink-0 mb-3">
        ...
        <div className="flex flex-col gap-2 pb-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              onClick={() => { setSelectedCategory(null); setSelectedRootCategory(null); }}
            ...
            {rootCategories.map((cat) => (
              <button
                key={cat.id}
                onClick={() => setSelectedRootCategory(cat.id)}
                className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${
                  selectedRootCategory === cat.id
                    ? 'bg-brand text-white'
          ...
          {/* Row 2: children of selected root */}
          {selectedRootCategory && (
            <div className="flex flex-wrap items-center gap-2 pl-2 border-s border-brand/20">
              <button
                onClick={() => setSelectedRootCategory(null)}
                className="px-3 py-1.5 rounded-lg text-xs font-medium text-muted-foreground hover:text-foreground transition-colors"
              >
                Back
              </button>
              {categories.filter((c) => c.parent_id === selectedRootCategory).map((cat) => (
                <button
                  key={cat.id}
                  onClick={() => setSelectedCategory(cat.id)}
                  className="px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors bg-card text-foreground border border-border hover:bg-muted"
                >
                  {cat.name}
                </button>
              ))}
