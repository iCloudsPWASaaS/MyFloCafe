...
function LoginContent() {
  ...
  const { login, loginByPin, selectTenant, user, tenants, currentTenant, loadFromStorage } = useAuthStore();
    ...
    try {
      ...
      if (err instanceof StorageUnavailableError) {
        ...
        if (failure.status === 401) {
          ...
          if (remaining === 0) {
            // Just got locked out
            const mins = failure.lockoutMinutes ?? 1;
          ...
          setLoginError(t('lockedOut', { minutes: 1 }));
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background px-4">
      <div className="w-full max-w-md">
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="mb-4 grid grid-cols-2 gap-2 rounded-lg border border-border bg-muted/40 p-1">
              <button
                type="button"
                onClick={() => { setLoginMethod('credentials'); setLoginError(null); }}
                className={`rounded-md px-3 py-2 text-sm font-semibold transition-colors ${loginMethod === 'credentials' ? 'bg-card shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
              >
                {t('credentialsLogin')}
              </button>
              <button
                ...
                className={`flex items-center justify-center gap-1.5 rounded-md px-3 py-2 text-sm font-semibold transition-colors ${loginMethod === 'pin' ? 'bg-card shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
              >
                <Lock size={14} />
                {t('pinLogin')}
              </button>
            ...
            {loginMethod === 'credentials' ? (
              ...
              <PinPad
                ...
                loading={loading}
                error={loginError}
                pinLabel={t('pinLoginTitle')}
                clearLabel={t('clearPin')}
                backspaceLabel={t('backspacePin')}
