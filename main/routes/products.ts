 ...
 */
export async function searchSerpApiImages(query: string): Promise<SerpApiImageResult[]> {
  ...
  try {
    const response = await fetch(
      `${SERPAPI_ENDPOINT}?engine=google_images&q=${encodeURIComponent(query)}&api_key=${encodeURIComponent(apiKey)}`,
    ...
    return payload.images_results
      .map((item) => ({
        ...
        full: (item.original as string) ?? null,
        width: (item.original_width as number | undefined) ?? null,
        height: (item.original_height as number | undefined) ?? null,
        context_url: (item.link as string) ?? null,
      }))
...
router.get('/image-search', requireRole(...ROLE_ACCESS.ownerManager), asyncHandler(async (req: Request, res: Response) => {
