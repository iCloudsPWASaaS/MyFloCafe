...
router.post('/login-pin', authRateLimit(), asyncHandler(async (req: Request, res: Response) => {
  try {
    const ip = req.ip || req.socket.remoteAddress || 'unknown';
    const rateLimit = checkRateLimit(ip);
    if (!rateLimit.allowed) {
    ...
    const matched = candidates.find((u: any) => verifyPin(u.pin_hash, pin));
    ...
    const token = jwt.sign(
      { userId: matched.id, email: matched.email, role: matched.role, remember, jti: randomUUID() },
      getJWTSecret(),
      { expiresIn: expiresInFor(remember) }
    );
