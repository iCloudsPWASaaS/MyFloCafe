...
router.get('/export/categories', requireRole(...ROLE_ACCESS.ownerManager), (_req: Request, res: Response) => {
  try {
    ...
    const sorter = (a: any, b: any) => (a.sort_order - b.sort_order) || a.name.localeCompare(b.name);
    const ordered: any[] = [];
    const walk = (list: any[]) => {
      for (const c of [...list].sort(sorter)) {
        ordered.push(c);
        if (childrenByParent.has(c.id)) walk(childrenByParent.get(c.id)!);
      }
    };
    walk(roots);
...
router.post('/import/categories', requireRole(...ROLE_ACCESS.ownerManager), (req: Request, res: Response) => {
  try {
    ...
    for (const existing of db.prepare('SELECT id, name FROM categories WHERE deleted_at IS NULL').all() as any[]) {
      knownIds.set(existing.name.toLowerCase(), existing.id);
    }

    type PendingRow = { r: Record<string, string>; rowNo: number; name: string };
    const pending: PendingRow[] = rows.map((r, index) => ({ r, rowNo: index + 2, name: (r.name || '').trim() }));
    ...
    db.transaction(() => {
      ...
      while (pending.length) {
        ...
        for (let i = pending.length - 1; i >= 0; i--) {
          ...
          db.prepare(
            `INSERT INTO categories (id, name, slug, description, color, icon, sort_order, parent_id, is_active, created_at, updated_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?)`
          ).run(id, name, slug, r.description || null, r.color || null, r.icon || null,
            sortOrder.value, parentId, now(), now());
