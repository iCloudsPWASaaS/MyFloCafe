...
import { computeShiftNetCollections } from './shifts';
...
function userNameById(db: ReturnType<typeof getDatabase>, ids: Array<string | null | undefined>): Record<string, string> {
  const unique = [...new Set(ids.filter((id): id is string => !!id))];
  if (!unique.length) return {};
  const rows = db.prepare(`SELECT id, name FROM users WHERE id IN (${unique.map(() => '?').join(',')})`).all(...unique) as Array<{ id: string; name: string }>;
...
router.get('/z-report', requireRole(...ROLE_ACCESS.owner), (req: Request, res: Response) => {
  try {
    const db = getDatabase();
    const minorFactor = getCurrencyMinorUnitFactor(getTenantCurrency(db));
    const today = reportToday();
    ...
    `).get(start, end) as { count: number; subtotal: number; discounts: number; tax: number; total: number };
    ...
    const collections = db.prepare(`
      ...
      FROM bills WHERE paid_at >= ? AND paid_at < ?
    `).get(start, end) as { count: number; gross_collected: number };
    const refundTotals = db.prepare(`
      SELECT COUNT(*) AS count, COALESCE(SUM(CAST(r.amount_cents AS REAL)) / ?, 0) AS refunded
      FROM refunds r JOIN bills b ON b.id = r.bill_id
      WHERE b.paid_at >= ? AND b.paid_at < ?
    `).get(minorFactor, start, end) as { count: number; refunded: number };
    ...
    const ordersByStatus = db.prepare(`
      ...
      FROM orders WHERE created_at >= ? AND created_at < ?
      GROUP BY status
    `).all(start, end) as { status: string; count: number }[];

    const orderTypes = db.prepare(`
      SELECT type, COUNT(*) AS count, COALESCE(SUM(total), 0) AS total
      FROM orders WHERE created_at >= ? AND created_at < ?
      GROUP BY type
    `).all(start, end) as { type: string; count: number; total: number }[];
    ...
    const billOrders = getOrdersWithItemsForBills(db, dayBills);
    ...
    const shifts = dayShifts.map((s) => {
      ...
      const expectedCash = openingCash + computeShiftNetCollections(db, s);
    ...
    const sum = (rows: typeof shifts, key: 'opening_cash' | 'closing_cash' | 'expected_cash' | 'variance') =>
      rows.reduce((acc, s) => acc + Number(s[key] || 0), 0);
    ...
    res.json({
      zReport: {
        ...
        ordersByStatus,
        orderTypes,
        taxComponents: aggregateTaxComponents(documents),
        shifts,
        shiftTotals: {
          count: shifts.length,
